...
import { JOHN_SPIRIT_ATTACK_CLIP, JOHN_SPIRIT_ATTACK_DAMAGE_FRAME, JOHN_SPIRIT_ATTACK_DAMAGE_SECONDS, johnAnimationAvailableInForm, johnSpiritMovementDuration, johnSpiritPlaybackRate } from '../src/johnChristLocomotion.ts';
...
assert.equal(spiritVisualDesired(false, false, undefined, false, undefined, true), true, 'A Spirit attack holds Spirit Form until its complete clip finishes.');
...
applySpiritBlend(normal,spirit,body,1);assert(!normal.visible && spirit.visible);assert.equal(spirit.material.opacity,0.7);assert.equal(body.scale.x,1,'Spirit Form matches normal-form size.');
...
const spiritAttack=asset.animations.find(a=>a.name===JOHN_SPIRIT_ATTACK_CLIP)!;
assert(spiritAttack && spiritAttack.duration > JOHN_SPIRIT_ATTACK_DAMAGE_SECONDS);
assert.equal(JOHN_SPIRIT_ATTACK_CLIP, 'Skill_01');
assert.equal(JOHN_SPIRIT_ATTACK_DAMAGE_FRAME, 12);
assert.equal(JOHN_SPIRIT_ATTACK_DAMAGE_SECONDS, 11 / 24);
assert.equal(johnAnimationAvailableInForm('Attack', true), true);
assert.equal(johnAnimationAvailableInForm('Blessing', true), false);
assert.equal(johnAnimationAvailableInForm('Cleanse', true), false);
assert.equal(johnAnimationAvailableInForm('MindBlast', true), false);
assert.equal(johnAnimationAvailableInForm('Blessing', false), true);
