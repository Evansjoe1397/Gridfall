...
for (const simultaneousCombatStack of [false, true]) {
  for (const testCase of [
    ...
    assert.equal(redirected.objects.some((object) => object.id === 'permanent-wall'), true, 'Redirect does not destroy permanent arena walls.');
...
console.log("Redirect object checks passed: Da Orkk's Shield and Wreckna's Tomb are destructible in both combat-stack modes.");
