...
export const CharacterIdSchema = z.enum(['shinobi', 'orkk', 'magician', 'john-christ', 'spectre', 'wreckna', 'merylin']);
...
export const GameCommandSchema = z.discriminatedUnion('type', [
  ...
  z.object({ type: z.literal('sweet-potato-choice'), playerId: PlayerIdSchema, choice: z.enum(['heal', 'cleanse', 'cancel']) }),
  z.object({ type: z.literal('frostmourne-decision'), playerId: PlayerIdSchema, use: z.boolean() }),
  z.object({ type: z.literal('lightbringer-swap-decision'), playerId: PlayerIdSchema, swap: z.boolean() }),
  ...
  z.object({ type: z.literal('spectre-perk-origin-select'), playerId: PlayerIdSchema, origin: z.enum(['spectre', 'replica']), replicaId: z.string().nullable().optional() }),
  ...
  z.object({ type: z.literal('boomerang-target'), playerId: PlayerIdSchema, targetId: z.string(), targetKind: z.enum(['player', 'replica']).optional() }),
  ...
  z.object({ type: z.literal('oracle-reveal'), playerId: PlayerIdSchema }),
  z.object({ type: z.literal('yamato-move'), playerId: PlayerIdSchema, to: CellSchema.nullable() }),
...
export const CARDS: readonly Card[] = [
  ...
  { id: 'feint', name: 'Feint', kind: 'attack', value: 2, effectText: 'Before combat: cancel the Defend Card effects, unless this effect is blocked. Removed on use or Discard.' },
  ...
  { id: 'banner', name: 'The Banner', kind: 'status', value: 1, effectText: '+1 MOV while this Card is in Hand. Apply as +1 in Combat, then Remove this Card. Automatically Discard this Card whenever you are required to Discard a Card.', canDiscardForHandLimit: true },
  { id: 'banner-draw', name: 'The Banner', kind: 'status', value: 1, effectText: '+1 MOV while this Card is in Hand. Automatically Discard this Card whenever you are required to Discard a Card.', canDiscardForHandLimit: true },
  { id: 'helmet', name: 'Helmet', kind: 'status', value: 2, effectText: 'Combat Card: add +2 to the Defend Value of your played Card. Then, Remove from the Deck.' },
  ...
  { id: 'boomerang-draw', name: 'Boomerang', kind: 'free-action', value: 1, effectText: 'Draw Reward. -1 MOV while not in Hand. Use as a Free Action for 1 Damage at Range 3, or automatically use as an Action for 2 Damage at melee Range 1. Shuffle this Card into your Deck after use.' },
...
const SPECTRE_CARD_IDS: readonly CardTypeId[] = ['replicate', 'relocate', 'shadow-dagger', 'consume-replica', 'haunt', 'solitude', 'deja-vu', 'echo-strike', 'soul-strike', 'displace', 'devour', 'split', 'anguish', 'dispersion', 'accumulate'];
const WRECKNA_CARD_IDS: readonly CardTypeId[] = ['hex', 'shadow-barter', 'enfeeble', 'finger-of-death', 'bone-chill', 'tomb-block', 'brain-freeze', 'sacrifice', 'immortality', 'graveyard', 'sap', 'dakkoth', 'lichdom', 'necronomicon', 'decay'];
const MERYLIN_CARD_IDS: readonly CardTypeId[] = ['carian-stance', 'excalibur', 'moonlight', 'sting', 'lightbringer', 'frostmourne', 'decisive-block', 'redirect', 'tactician', 'yamato', 'oracle', 'windwalker-stance', 'barbarian-stance', 'kamelot-stance', 'spellsinger-stance'];
...
export const STARTING_DECKS: Record<CharacterId, StartingDeckDefinition> = {
  ...
  spectre: {
    defaults: ['displace', 'deja-vu', 'echo-strike', 'devour', 'split', 'anguish', 'replicate', 'relocate', 'shadow-dagger'],
    reserve: 'replicate', attackFocus: ['soul-strike', 'solitude'], defendFocus: ['dispersion', 'accumulate'], perkPhase: ['consume-replica', 'haunt'],
  },
  wreckna: {
...
export type PlayerState = {
  ...
  actionsRemaining: number; perkUsed: boolean; freeMoveUsed: boolean; movementRemaining: number; movementSpentThisTurn?: number;
...
function createInitialStateWithPlaceholder(lineup: 'orkk-vs-dummy' | 'shinobi-vs-orkk' = 'orkk-vs-dummy'): GameState {
  ...
  const objects: BoardObject[] = [
    ...NAGRAND_ARENA.pillars.map((label, index) => ({ id: `nagrand-pillar-${index + 1}`, name: 'Pillar - Nagrand', kind: 'wall-pillar' as const, hp: 999, maxHp: 999, position: cellFromLabel(label) })),
...
function characterDisplayName(character: PlayerState['character']): string {
  ...
  if (character === 'merylin') return 'Merylin Pendragon';
  return 'Test Dummy';
}
...
export function createTrenchTestState(includeAllCharacterCards = false, playerCharacter: HotseatCharacterId = 'magician', opponentCharacter: HotseatCharacterId | 'dummy' = 'dummy'): GameState {
  ...
  state.objects = [
    ...THE_TRENCH_ARENA.pillars.map((label, index) => ({
      id: `trench-column-${index + 1}`, name: 'Pillar - Lordaeron', kind: 'wall-pillar' as const,
    ...
    ...boxSpawns.map((label, index) => ({
      ...
      hp: 3, maxHp: 3, position: cellFromLabel(label), respawnEligible: true,
    })),
  ];
...
export const ACTION_QUEST_POOL: readonly ActionQuestDefinition[] = [
  ...
  { id: 'the-spy', name: 'The Spy', condition: 'Finish the most turns on an enemy Base Square during the next 4 Rounds.', reward: 'Feint', durationRounds: 4, determineWinners: progressLeaders, grantReward: (state, winnerId) => state.players[winnerId].hand.push({ instanceId: `${winnerId}-${++instanceSequence}`, cardId: 'feint', revealedToOpponent: false }) },
  { id: 'hot-potato', name: 'The Hot Potato', condition: 'Steal Potato with a successful Attack. Last one with Potato in 4 Rounds wins.', reward: 'Sweet Potato', durationRounds: 4, determineWinners: (state) => { const carrierId = questPhases(state).hotPotato?.carrierId; return carrierId && state.players[carrierId]?.hp > 0 ? [carrierId] : []; } },
];
export type QuestPhaseState = {
  ...
  objectRespawns?: { dueRound: number | null }[];
...
function resolveCaptureTheFlagAtTurnStart(state: GameState) {
  const phases = questPhases(state);
  if (phases.currentQuest?.id !== 'capture-the-flag' || !phases.captureTheFlag) return;
...
function recordQuestMovement(state: GameState, playerId: PlayerId, amount: number, teleport = false, destination?: Cell) {
  ...
  if (destination && !teleport) collectHotPotato(state, playerId, destination);
...
export function spectreReplicas(state: GameState, ownerId: PlayerId): BoardObject[] {
  return state.objects.filter((object) => object.kind === 'spectre-replica' && object.ownerId === ownerId);
}

type CharacterBody = { kind: 'player' | 'replica'; id: string; ownerId: PlayerId; position: Cell; name: string };

function characterBodies(state: GameState): CharacterBody[] {
  const players = Object.values(state.players)
    .filter((player) => player.hp > 0)
    .map((player): CharacterBody => ({ kind: 'player', id: player.id, ownerId: player.id, position: player.position, name: player.name }));
  const replicas = state.objects
    .filter((object) => object.kind === 'spectre-replica' && object.ownerId && state.players[object.ownerId]?.hp > 0)
    .map((object): CharacterBody => ({ kind: 'replica', id: object.id, ownerId: object.ownerId!, position: object.position, name: object.name }));
  return [...players, ...replicas];
}

function enemyBodies(state: GameState, ownerId: PlayerId): CharacterBody[] {
  return characterBodies(state).filter((body) => body.ownerId !== ownerId);
}

function characterBody(state: GameState, kind: 'player' | 'replica' | undefined, id: string): CharacterBody | undefined {
  return characterBodies(state).find((body) => body.kind === (kind ?? 'player') && body.id === id);
}
export function isShadowCloakedPerkTarget(state: GameState, kind: 'player' | 'replica' | 'object', id: string): boolean {
  const replica = kind === 'player' ? undefined : state.objects.find((object) => object.id === id && object.kind === 'spectre-replica');
  const owner = kind === 'player' ? state.players[id as PlayerId] : replica?.ownerId ? state.players[replica.ownerId] : undefined;
  return Boolean(owner?.spectreShadowCloakActive);
}

function enemyBodyAt(state: GameState, ownerId: PlayerId, cell: Cell): CharacterBody | undefined {
...
function pushEntityForBody(body: CharacterBody): PushEntity {
  return { kind: body.kind === 'replica' ? 'object' : 'player', id: body.id, position: body.position };
}
function attackedSpectreReplica(state: GameState, pending: PendingAttack): BoardObject | undefined {
  if (pending.defenderBody !== 'replica') return undefined;
  const replicas = spectreReplicas(state, pending.defenderId);
  return replicas.find((replica) => replica.id === pending.defenderReplicaId)
    ?? replicas.find((replica) => pending.defenderPosition && replica.position.x === pending.defenderPosition.x && replica.position.y === pending.defenderPosition.y)
    ?? replicas[0];
}
function attackingSpectreReplica(state: GameState, pending: PendingAttack): BoardObject | undefined {
  if (pending.attackerBody !== 'replica') return undefined;
  const replicas = spectreReplicas(state, pending.attackerId);
  return replicas.find((replica) => replica.id === pending.attackerReplicaId)
    ?? replicas.find((replica) => pending.attackerPosition && replica.position.x === pending.attackerPosition.x && replica.position.y === pending.attackerPosition.y)
    ?? replicas[0];
}
function echoStrikeVictims(state: GameState, ownerId: PlayerId): CharacterBody[] {
  const replicas = spectreReplicas(state, ownerId);
  return enemyBodies(state, ownerId).filter((body) =>
    replicas.some((replica) => distance(body.position, replica.position) === 1)
  );
}
function createSpectreReplica(state: GameState, ownerId: PlayerId, position: Cell, spectreOnBoxId: string | null = null): BoardObject {
  const replica: BoardObject = { id: `${ownerId}-spectre-replica-${++instanceSequence}`, name: `${state.players[ownerId].name}'s Replica`, kind: 'spectre-replica', ownerId, hp: 999, maxHp: 999, position: { ...position }, spectreOnBoxId };
...
function createOrReplaceSpectreReplica(state: GameState, ownerId: PlayerId, position: Cell, spectreOnBoxId: string | null = null): BoardObject {
  state.objects = state.objects.filter((object) => object.kind !== 'spectre-replica' || object.ownerId !== ownerId);
  return createSpectreReplica(state, ownerId, position, spectreOnBoxId);
}
function segmentIntersectsTileInterior(from: Cell, to: Cell, tile: Cell): boolean {
  const dx = to.x - from.x;
  const dy = to.y - from.y;
  if (dx === 0 && dy === 0) return false;
  let entry = 0;
  let exit = 1;
  for (const [origin, delta, min, max] of [[from.x, dx, tile.x - 0.5, tile.x + 0.5], [from.y, dy, tile.y - 0.5, tile.y + 0.5]] as const) {
    if (delta === 0) {
...
function enemiesBetweenCells(state: GameState, playerId: PlayerId, from: Cell, to: Cell): CharacterBody[] {
  return enemyBodies(state, playerId).filter((enemy) => segmentIntersectsTileInterior(from, to, enemy.position));
}

function characterBodyKey(body: CharacterBody): string {
  return `${body.kind}:${body.id}`;
}

function resolveRelocateTetherMovementDamage(state: GameState, player: PlayerState, origin: Cell, movementCells: Cell[]): void {
  const replicaId = player.spectreRelocateTetherReplicaId;
  if (player.character !== 'spectre' || !replicaId || movementCells.length === 0) return;
  const replica = state.objects.find((object) => object.id === replicaId && object.kind === 'spectre-replica' && object.ownerId === player.id);
  if (!replica) return;
  let intersecting = new Set(enemiesBetweenCells(state, player.id, origin, replica.position).map(characterBodyKey));
  const damagedNames: string[] = [];
  for (const cell of movementCells) {
    const victims = enemiesBetweenCells(state, player.id, cell, replica.position);
    for (const victim of victims) {
      const key = characterBodyKey(victim);
      if (intersecting.has(key)) continue;
      damageCharacterBody(state, victim, 1, false, player.id, 'perk');
      damagedNames.push(victim.name);
    }
    intersecting = new Set(victims.map(characterBodyKey));
  }
  if (damagedNames.length > 0) state.log.unshift(`Relocate's moving tether dealt 1 Damage to ${damagedNames.join(', ')} when it newly intersected ${damagedNames.length === 1 ? 'their tile' : 'their tiles'}.`);
}
...
function destroySpectreReplicaById(state: GameState, ownerId: PlayerId, replicaId: string): Cell | null {
  const replica = state.objects.find((object) => object.id === replicaId && object.kind === 'spectre-replica' && object.ownerId === ownerId);
  if (!replica) return null;
  state.objects = state.objects.filter((object) => object.id !== replica.id);
  return { ...replica.position };
}
type SpectreTargetingState = GameState & {
  ...
  spectrePerkOrigin?: { casterId: PlayerId; perkId: 'shadow-dagger' | 'relocate' | 'devour'; level: number; origin: 'spectre' | 'replica'; replicaId: string | null; undo: PerkTargetingUndo | null } | null;
  ...
  pendingAnguish?: { defenderId: PlayerId; attackerId: PlayerId; sufferedDamage: boolean } | null;
  pendingSplit?: { defenderId: PlayerId; origin: Cell } | null;
};
export function activeWrecknaPhylactery(state: GameState, playerId: PlayerId, type: PhylacteryType): BoardObject | undefined {
  if (state.players[playerId]?.traitBlocked) return undefined;
...
export function spiritGuardianEnemyPenalty(state: GameState, player: PlayerState): number {
  if (player.spectreShadowCloakActive) return 0;
...
function activateNextObjectRespawn(state: GameState): number | null {
  const queue = questPhases(state).objectRespawns!;
  const next = queue[0];
  if (!next || next.dueRound !== null) return null;
  const delay = randomObjectRespawnDelay();
  next.dueRound = state.turn + delay;
  return delay;
}

function enqueueObjectRespawn(state: GameState): number | null {
  const queue = questPhases(state).objectRespawns!;
  queue.push({ dueRound: null });
  return queue.length === 1 ? activateNextObjectRespawn(state) : null;
}
...
function destroyObject(state: GameState, objectId: string, playerId: PlayerId, reason: string): boolean {
  ...
  if (destroyed.kind === 'wooden-box') {
    ...
    const replicasOnBox = state.objects.filter((object) => object.kind === 'spectre-replica' && object.spectreOnBoxId === destroyed.id);
    for (const replica of replicasOnBox) {
      replica.spectreOnBoxId = null;
      const owner = replica.ownerId ? state.players[replica.ownerId] : undefined;
      if (owner?.hp) {
        dealDamage(state, owner, 1, false, playerId, 'other');
        state.log.unshift(`${owner.name}'s Replica fell from the destroyed Box; ${owner.name} received 1 Damage.`);
      }
    }
  }
  ...
  if (destroyed.phylacteryType && destroyed.phylacteryOwnerId) state.log.unshift(`${state.players[destroyed.phylacteryOwnerId].name}'s Phylactery of ${destroyed.phylacteryType[0].toUpperCase()}${destroyed.phylacteryType.slice(1)} was deactivated.`);
  const respawnDelay = destroyed.respawnEligible ? enqueueObjectRespawn(state) : null;
  const respawnMessage = !destroyed.respawnEligible
    ? ''
    : respawnDelay === null
      ? ' It entered the respawn queue and will receive a timer after the Objects ahead of it return.'
      : ` It will respawn in ${respawnDelay} Rounds.`;
  state.log.unshift(`${state.players[playerId].name} destroyed ${destroyed.name} at ${cellLabel(destroyed.position)}${reason ? ` with ${reason}` : ''}.${respawnMessage}`);
  return true;
}
...
function ensureMatchStats(player: PlayerState): MatchStats {
  ...
  stats.objectsDestroyed ??= 0;
  return stats;
}

function collectHotPotato(state: GameState, playerId: PlayerId, destination: Cell) {
  const phases = questPhases(state);
  const potato = phases.currentQuest?.id === 'hot-potato' ? phases.hotPotato : null;
  if (!potato || potato.carrierId) return;
  const adjacentSquares = [
    { x: Math.floor(potato.anchor.x), y: Math.floor(potato.anchor.y) },
    { x: Math.ceil(potato.anchor.x), y: Math.floor(potato.anchor.y) },
    { x: Math.floor(potato.anchor.x), y: Math.ceil(potato.anchor.y) },
    { x: Math.ceil(potato.anchor.x), y: Math.ceil(potato.anchor.y) },
  ];
  if (!adjacentSquares.some((cell) => cell.x === destination.x && cell.y === destination.y)) return;
  const player = state.players[playerId];
  player.hand.push({ instanceId: `${playerId}-hot-potato-${++instanceSequence}`, cardId: 'hot-potato', revealedToOpponent: true });
  potato.carrierId = playerId;
  state.log.unshift(`${player.name} picked up the Hot Potato.`);
}

function removeHotPotatoCard(player: PlayerState) {
  player.hand = player.hand.filter((card) => card.cardId !== 'hot-potato');
  player.deck = player.deck.filter((card) => card.cardId !== 'hot-potato');
  player.discard = player.discard.filter((card) => card.cardId !== 'hot-potato');
}
...
function showStatEffect(state: GameState, player: PlayerState, amount: number, stat: 'MOV' | 'ATT' | 'DEF') {
  if (!amount) return;
  state.objectPushAnimations.push({
    id: `${state.turn}-stat-${player.id}-${stat}-${state.log.length}-${state.objectPushAnimations.length}`,
    objectId: '',
    ...
    dy: 0,
    collided: false,
    statEffect: { playerId: player.id, amount, stat },
  });
}

function showLightsaberStats(state: GameState, player: PlayerState) {
  showStatEffect(state, player, 1, 'ATT');
  showStatEffect(state, player, 1, 'DEF');
  showStatEffect(state, player, 1, 'MOV');
}

export function determineTimedMatchWinner(state: GameState): PlayerId | null {
  const candidates = (Object.keys(state.players) as PlayerId[]).filter((id) => state.players[id].hp > 0 && state.players[id].character !== 'dummy');
  const ordered = [...candidates].sort((leftId, rightId) => {
    const left = state.players[leftId];
    const right = state.players[rightId];
    const leftStats = ensureMatchStats(left);
    const rightStats = ensureMatchStats(right);
    return rightStats.totalDamage - leftStats.totalDamage
      || rightStats.attackDamage - leftStats.attackDamage
      || rightStats.perkDamage - leftStats.perkDamage
      || right.hp - left.hp
      || rightStats.objectsDestroyed - leftStats.objectsDestroyed
      || rightStats.squaresMoved - leftStats.squaresMoved
      || candidates.indexOf(leftId) - candidates.indexOf(rightId);
  });
  return ordered[0] ?? null;
}

function finishTimedMatch(state: GameState): boolean {
  const winnerId = determineTimedMatchWinner(state);
  state.winner = winnerId;
  state.phase = 'finished';
  if (winnerId) {
    const winner = state.players[winnerId];
    const stats = ensureMatchStats(winner);
    state.log.unshift(`Five Action Quests are complete. ${winner.name} wins by match statistics with ${stats.totalDamage} Total Damage.`);
  } else state.log.unshift('Five Action Quests are complete, but no eligible Player remains.');
  return false;
}
...
function announceActionQuest(state: GameState, round: number): boolean {
  ...
  if (selected.id === 'capture-the-flag') {
    ...
    // A character may already be occupying an enemy Base square when this
    // quest is announced, so evaluate their current position as well as
    // later movement destinations.
    for (const player of Object.values(state.players)) {
      if (player.hp > 0 && player.character !== 'dummy') updateCaptureTheFlag(state, player.id, player.position);
    }
  } else questState.captureTheFlag = null;
  if (selected.id === 'hot-potato') questState.hotPotato = { anchor: { x: (boardWidth(state) + 1) / 2, y: (boardHeight(state) - 1) / 2 }, carrierId: null };
  else questState.hotPotato = null;
...
function resolveCurrentActionQuest(state: GameState): void {
  ...
  if (active) {
    ...
    for (const winnerId of questState.lastQuestWinners) {
      if (tied) {
        if (drawRewardId) state.players[winnerId].hand.push({ instanceId: `${winnerId}-${++instanceSequence}`, cardId: drawRewardId, revealedToOpponent: drawRewardId === 'weak-feint' ? false : true, oneTimeCopy: drawRewardId === 'monarch-flush-perk' });
        continue;
      }
      ...
      if (active.id === 'hot-potato') {
        ...
        state.players[winnerId].hand.push({ instanceId: `${winnerId}-${++instanceSequence}`, cardId: 'sweet-potato', revealedToOpponent: true });
      }
    }
...
function beginOpeningSetup(state: GameState, playerIds: PlayerId[], after: 'active' | 'placement') {
  ...
  setupState.openingSetup = { pendingPlayerIds: [...playerIds], focusByPlayer: {}, after, firstPlayerId: state.activePlayerId, roundOrder };
  questPhases(state);
...
function canChooseOpeningFocus(opening: OpeningSetupState, playerId: PlayerId): boolean {
  return opening.after === 'placement' ? opening.pendingPlayerIds.includes(playerId) : opening.pendingPlayerIds[0] === playerId;
}
...
function resolveFocusChoice(state: GameState, playerId: PlayerId, focus: 'attack' | 'defend'): CommandResult {
  ...
  if (state.phase !== 'choosing-focus' || !opening || !canChooseOpeningFocus(opening, playerId)) return fail(state, 'This Player is not choosing Focus now.');
  opening.focusByPlayer[playerId] = focus;
  state.phase = opening.after === 'placement' ? 'choosing-focus' : 'choosing-focus-card';
...
function returnToFocusChoice(state: GameState, playerId: PlayerId): CommandResult {
  const opening = (state as GameStateWithOpening).openingSetup;
  if (!['choosing-focus', 'choosing-focus-card'].includes(state.phase) || !opening || !canChooseOpeningFocus(opening, playerId) || !opening.focusByPlayer[playerId]) return fail(state, 'This Player cannot return to Focus choice now.');
...
function drawAdditionalOpeningCard(player: PlayerState): number {
  const focusCard = player.deck.pop();
  const drawn = drawCards(player, 1);
  if (focusCard) player.deck.push(focusCard);
  player.knownTopCardId = focusCard?.cardId ?? null;
  return drawn;
}
...
function resolveFocusCardChoice(state: GameState, playerId: PlayerId, cardId: CardTypeId): CommandResult {
  ...
  let focus = opening?.focusByPlayer[playerId];
  if (state.phase === 'choosing-focus' && opening && canChooseOpeningFocus(opening, playerId)) {
    const player = state.players[playerId];
    if (focusCandidates(player, 'attack').includes(cardId)) focus = 'attack';
    else if (focusCandidates(player, 'defend').includes(cardId)) focus = 'defend';
    if (focus) opening.focusByPlayer[playerId] = focus;
  }
  if (!['choosing-focus', 'choosing-focus-card'].includes(state.phase) || !opening || !canChooseOpeningFocus(opening, playerId) || !focus) return fail(state, 'This Player is not choosing a Focus Card now.');
  ...
  opening.pendingPlayerIds = opening.pendingPlayerIds.filter((pendingPlayerId) => pendingPlayerId !== playerId);
  ...
  if (opening.pendingPlayerIds.length > 0) {
    ...
    for (const laterPlayerId of opening.roundOrder.slice(1)) {
      const laterPlayer = state.players[laterPlayerId];
      if (!laterPlayer) continue;
      const drawn = drawAdditionalOpeningCard(laterPlayer);
      state.log.unshift(`${laterPlayer.name} does not go first in Round 1 and drew ${drawn} additional opening Card${drawn === 1 ? '' : 's'} while keeping the selected Focus Card on top of the Deck.`);
    }
...
export function phaseCardCandidates(state: GameState, playerId: PlayerId): CardTypeId[] {
  ...
  const progression = questPhases(state).progression[playerId];
  const initialFocus = progression?.initialFocus ?? 'attack';
  const oppositeFocusCards = initialFocus === 'attack' ? definition.defendFocus : definition.attackFocus;
  const remainingInitialFocusCard = focusCandidates(player, initialFocus).filter((cardId) => cardId !== progression?.chosenFocusCard);
...
function createPlayer(id: PlayerId, name: string, character: PlayerState['character'], position: Cell): PlayerState {
  const uniqueIds = character === 'shinobi' ? OBI_WAN_CARD_IDS : character === 'orkk' ? DA_ORKK_CARD_IDS : character === 'magician' ? LOGAN_CARD_IDS : character === 'john-christ' ? JOHN_CHRIST_CARD_IDS : character === 'spectre' ? SPECTRE_CARD_IDS : character === 'wreckna' ? WRECKNA_CARD_IDS : character === 'merylin' ? MERYLIN_CARD_IDS : [];
...
function applySwiftformPinnedOnce(state: GameState, player: PlayerState, enemyId: PlayerId): boolean {
  ...
  if (state.players[enemyId].spectreShadowCloakActive) return false;
  applyPinned(state.players[enemyId], 1, state, player.id);
  player.swiftformPinnedEnemyIds.push(enemyId);
  return true;
}

function resolveMoonlightWave(state: GameState, attacker: PlayerState, attackerPosition: Cell, targetPosition: Cell) {
  const dx = Math.sign(targetPosition.x - attackerPosition.x);
  const dy = Math.sign(targetPosition.y - attackerPosition.y);
  if (dx === 0 && dy === 0) return;
  const waveCells = [1, 2]
    .map((step) => ({ x: targetPosition.x + dx * step, y: targetPosition.y + dy * step }))
    .filter((cell) => cell.x >= 1 && cell.x <= boardWidth(state) && cell.y >= 0 && cell.y < boardHeight(state));
  if (waveCells.length === 0) {
    state.log.unshift('Moonlight formed at the Board edge and found no Square behind its target.');
    return;
  }
  ...
  for (const [index, cell] of waveCells.entries()) {
    ...
    for (const object of objects) {
      ...
      else destroyObject(state, object.id, attacker.id, 'with Moonlight\'s moonwave');
    }
...
function resolveStingAfterCombat(state: GameState, attacker: PlayerState, cardInstanceId: string) {
  const nearbyEnemy = enemyBodies(state, attacker.id).some((body) => distance(attacker.position, body.position) <= 2);
  if (nearbyEnemy) {
    grantMerylinSummon(state, attacker.id, 'Sting');
    const drawn = drawCards(attacker, 1);
    state.log.unshift(`Sting found an enemy Character within 2 Squares; ${attacker.name} gained Summon and drew ${drawn} Card.`);
    return;
  }
...
function beginFrostmourneChoice(state: GameState, attacker: PlayerState, cardInstanceId: string, resumePhase: GamePhase): boolean {
  if (attacker.hp <= 1 || !attacker.discard.some((card) => card.instanceId === cardInstanceId && card.cardId === 'frostmourne')) {
    if (attacker.hp <= 1) state.log.unshift(`${attacker.name} cannot sacrifice 1 Hit Point for Frostmourne while at ${attacker.hp} HP.`);
    return false;
  }
  (state as FrostmourneChoiceState).frostmourne = { playerId: attacker.id, cardInstanceId, resumePhase };
  state.phase = 'choosing-frostmourne';
  state.log.unshift(`${attacker.name} may sacrifice 1 Hit Point to put Frostmourne on top of the Deck, gain 1 Action, and Summon.`);
  return true;
}

function resolveFrostmourneDecision(state: GameState, playerId: PlayerId, use: boolean): CommandResult {
  const extended = state as FrostmourneChoiceState;
  const choice = extended.frostmourne;
  if (state.phase !== 'choosing-frostmourne' || !choice || choice.playerId !== playerId) return fail(state, 'Frostmourne is not waiting for this Player.');
  const player = state.players[playerId];
  if (use && player.hp <= 1) return fail(state, 'Frostmourne requires more than 1 Hit Point to pay its sacrifice.');
  const resumePhase = choice.resumePhase;
  extended.frostmourne = null;
  if (use) {
    const discardIndex = player.discard.findIndex((card) => card.instanceId === choice.cardInstanceId && card.cardId === 'frostmourne');
    if (discardIndex < 0) return fail(state, 'Frostmourne is no longer in this Player\'s Discard.');
    dealDamage(state, player, 1, false, player.id, 'other');
    const [frostmourne] = player.discard.splice(discardIndex, 1);
    frostmourne.revealedToOpponent = false;
    player.deck.push(frostmourne);
    player.knownTopCardId = 'frostmourne';
    player.actionsRemaining += 1;
    grantMerylinSummon(state, player.id, 'Frostmourne');
    state.log.unshift(`${player.name} sacrificed 1 Hit Point, put Frostmourne on top of the Deck, gained 1 Action, and Summoned.`);
  } else state.log.unshift(`${player.name} left Frostmourne in Discard.`);
...
function activateLightTheSaber(state: GameState, attacker: PlayerState) {
  ...
  const newlyActive = !attacker.lightsaberBuff && !attacker.traitBlocked;
  ...
  if (newlyActive) showLightsaberStats(state, attacker);
...
function resolveObjectAttack(state: GameState, player: PlayerState, instance: CardInstance, objectId: string, attackOrigin: Cell = player.position, attackRange = effectiveAttackRange(state, player), lightbringerSwap?: boolean): CommandResult {
  ...
  if (card.id === 'excalibur' ? !attackCardTargetInRange(state, player, card.id, object.position) : distance(attackOrigin, object.position) > attackRange) {
    return fail(state, card.id === 'excalibur'
      ? "Object is outside Excalibur's Range 2 or not in a direct line."
      : `Object is outside ${card.name}'s attack range.`);
  }
  ...
  const attackingReplica = state.objects.find((entry) => entry.kind === 'spectre-replica' && entry.ownerId === player.id && entry.position.x === attackOrigin.x && entry.position.y === attackOrigin.y);
  const attackOnBoxId = attackOrigin.x === player.position.x && attackOrigin.y === player.position.y ? player.spectreOnBoxId : attackingReplica?.spectreOnBoxId;
  const baseHighGroundBonus = highGroundAttackValueBonus(state, { ...player, position: attackOrigin, spectreOnBoxId: attackOnBoxId }, object.position);
  ...
  const attackValue = (card.id === 'drain-strength' ? 1 : card.value)
    ...
    + rageSpent
    + (player.character === 'john-christ' && player.spiritForm && !player.traitBlocked ? 2 : 0)
  ...
  if (card.id === 'blessed-might') {
    addBlessingCardToJohn(state, player, 'blessing-might', 'attack', !player.spiritForm);
  ...
  if (card.id === 'repent') {
    ...
    state.spellProjectiles.push({ id: `${state.turn}-repent-target-${++instanceSequence}`, casterId: player.id, targetId: object.id, from: { ...object.position }, to: { ...object.position }, path: [], count: 1, damage: 0, style: 'cleanse-immolate' });
    const enemies = enemyBodies(state, player.id).filter((body) => distance(player.position, body.position) === 1);
  ...
  if (card.id === 'blessed-light' && player.character === 'john-christ') {
    addBlessingCardToJohn(state, player, 'blessing-light', 'attack', !player.spiritForm);
  ...
  if (card.id === 'arcane-bolt') {
    const previousBonus = player.arcaneBoltAttackBonus;
    player.arcaneBoltAttackBonus = player.manaMode === 'consume' ? 2 : 1;
    showStatEffect(state, player, Math.max(0, player.arcaneBoltAttackBonus - previousBonus), 'ATT');
  ...
  if (card.id === 'echo-strike') {
    ...
    const victims = echoStrikeVictims(state, player.id);
    for (const victim of victims) damageCharacterBody(state, victim, 1, false, player.id, 'attack');
    state.log.unshift(replicas.length > 0 ? `Echo Strike blasted ${victims.map((victim) => victim.name).join(', ') || 'no adjacent enemies'} around ${replicas.length === 1 ? 'the replica' : `${replicas.length} replicas`}.` : 'Echo Strike found no replica.');
  }
      ...
      if (recall) {
        ...
        if (path.length > 0) {
          ...
          for (const [pathIndex, cell] of path.entries()) {
            ...
            const enemyKey = enemy ? `${enemy.kind}:${enemy.id}` : '';
            if (!enemy || crossedEnemyIds.has(enemyKey)) continue;
            crossedEnemyIds.add(enemyKey);
            const damageAnimationStart = state.objectPushAnimations.length;
            damageCharacterBody(state, enemy, 2, true, player.id, 'attack');
...
function beginBoomerang(state: GameState, player: PlayerState, cardInstanceId: string): CommandResult {
  ...
  if (card.cardId !== 'boomerang' && card.cardId !== 'boomerang-draw') return fail(state, 'That Free Action is not supported.');
...
function shuffleDrawRewardBoomerangIntoDeck(player: PlayerState, card: CardInstance) {
  card.revealedToOpponent = false;
...
function resolveBoomerangTarget(state: GameState, playerId: PlayerId, targetId: string, targetKind: 'player' | 'replica' = 'player'): CommandResult {
  ...
  if (!body || !target || body.ownerId === playerId || target.hp <= 0) return fail(state, 'Choose a living enemy.');
  const targetDistance = distance(player.position, body.position);
  ...
  const index = player.hand.findIndex((entry) => entry.instanceId === pending.cardInstanceId && (entry.cardId === 'boomerang' || entry.cardId === 'boomerang-draw'));
  ...
  damageCharacterBody(state, body, damage, false, playerId, 'other');
  state.spellProjectiles.push({ id: `${state.turn}-boomerang-${++instanceSequence}`, casterId: playerId, targetId: body.id, from: { ...player.position }, to: { ...body.position }, path: [{ ...player.position }, { ...body.position }], count: 1, damage, style: 'boomerang' });
  if (!meleeUse && boomerang.cardId === 'boomerang-draw') shuffleDrawRewardBoomerangIntoDeck(player, boomerang);
  else if (!meleeUse) shuffleBoomerangIntoDeck(player, boomerang);
...
export function cardBaseValue(instance: CardInstance): number { return instance.cardId === 'oracle' ? Math.max(1, Math.min(3, instance.oracleValue ?? 3)) : cardDefinition(instance).value; }
...
export function attackCardTargetInRange(state: GameState, player: PlayerState, cardId: CardTypeId, target: Cell): boolean {
  const targetDistance = distance(player.position, target);
  if (cardId !== 'excalibur') return targetDistance <= effectiveAttackRange(state, player);
  if (targetDistance <= 1) return true;
  if (targetDistance !== 2) return false;
...
function tacticianTerrainEligible(state: GameState, position: Cell): boolean {
  ...
  const adjacentColumn = orthogonalCells.some((cell) => state.objects.some((object) => object.kind === 'wall-pillar' && object.position.x === cell.x && object.position.y === cell.y));
  return adjacentColumn || (!isHighGround(state, position) && orthogonalCells.some((cell) => isHighGround(state, cell)));
}
function tacticianDefenseBonus(state: GameState, defender: PlayerState, cardId: CardTypeId, position: Cell, effectsCancelled = false): number {
  return cardId === 'tactician' && defender.character === 'merylin' && !effectsCancelled && tacticianTerrainEligible(state, position) ? 2 : 0;
}
...
export function wrecknaPerkTargetInRange(state: GameState, caster: PlayerState, target: Cell, rangeOverride?: number): boolean {
  const range = rangeOverride ?? effectiveAttackRange(state, caster);
  return distance(caster.position, target) <= range && hasLineOfSight(state, caster.position, target);
}

export function grantMerylinSummon(state: GameState, playerId: PlayerId, source = 'a Card effect'): boolean {
  const player = state.players[playerId];
  if (!player || player.character !== 'merylin') return false;
  player.merylinSummonActive = true;
  state.log.unshift(`${player.name} gained Summon from ${source}; her next Attack Card is enabled.`);
  return true;
}

function consumeMerylinSummonForAttack(state: GameState, player: PlayerState) {
  if (player.character !== 'merylin') return;
  player.merylinSummonActive = false;
...
function spendAttackAction(player: PlayerState) {
  if (player.actionsRemaining > 0) player.actionsRemaining -= 1;
  else player.spellsingerExtraAttacks = Math.max(0, (player.spellsingerExtraAttacks ?? 0) - 1);
}
...
export function dealDamage(state: GameState, target: PlayerState, amount: number, collision = false, sourceId: PlayerId = state.activePlayerId, sourceKind: 'attack' | 'perk' | 'defense' | 'other' = 'other', effect = false): number {
  ...
  if (helmetProtectsCombatDamage) {
    ...
    state.log.unshift(`Mythril Helmet negated ${resolvedAmount} Damage to ${target.name}.`);
    return 0;
  }
  ...
  if (resolvedAmount > 0 && sourceKind === 'perk' && target.spectreShadowCloakActive) {
    state.log.unshift(`${target.name}'s Shadow Cloak negated ${resolvedAmount} Perk Damage.`);
    return 0;
  }
  ...
  if (dealt > 0 && target.character === 'john-christ') {
    if (target.stoicShell) {
      ...
      state.log.unshift(`${target.name}'s Stoic Shell was removed by Damage.`);
    }
    if (target.hp > 0 && !target.traitBlocked) {
...
type StatusDestination = 'hand' | 'deck' | 'discard';
...
function nextRedirectObject(state: GameState, defender: PlayerState): BoardObject | undefined {
  ...
  const preparedObjectIds = (redirect as typeof redirect & { preparedObjectIds?: string[] }).preparedObjectIds;
  if (preparedObjectIds) {
...
function combatDefenderProtectedFromNegativeEffects(state: GameState, target: PlayerState): boolean {
  ...
  return pending?.devourProtectionPlayerId === target.id || (pending?.blinkMissed === true && pending.defenderId === target.id);
}
function combatProtectionName(state: GameState): 'Blink' | 'Devour' {
  return (state.pendingAttack as BlinkPendingAttack | null)?.blinkMissed ? 'Blink' : 'Devour';
}
function redirectCombatStatusEffect(state: GameState, target: PlayerState, statusName: string, sourceId: PlayerId): boolean {
  const pending = state.pendingAttack;
  const redirect = pending?.redirect;
  if (!redirect || redirect.statusRedirected || target.id !== pending!.defenderId || sourceId !== pending!.attackerId) return false;
  const object = nextRedirectObject(state, target);
...
export function addForcedStatusCard(state: GameState, target: PlayerState, cardId: CardTypeId, destination: StatusDestination, sourceId: PlayerId = state.activePlayerId, sourceKind: 'attack' | 'perk' | 'defense' | 'other' = 'other', revealedToOpponent = destination !== 'deck', bypassDevourProtection = false): boolean {
  ...
  if (!bypassDevourProtection && combatDefenderProtectedFromNegativeEffects(state, target)) {
    state.log.unshift(`${combatProtectionName(state)} prevented ${cardDefinition({ instanceId: '', cardId }).name} from affecting ${target.name}.`);
    return false;
  }
  if (redirectCombatStatusEffect(state, target, cardDefinition({ instanceId: '', cardId }).name, sourceId)) return false;
  if (forcedStatusCount(target, cardId) >= 5) {
    const dealt = dealDamage(state, target, 1, false, sourceId, sourceKind);
    state.log.unshift(`${target.name} already had 5 ${cardDefinition({ instanceId: '', cardId }).name} Cards; the excess forced copy became ${dealt} Damage instead.`);
    return false;
  }
  ...
  if (destination === 'hand' && cardId === 'exhaust' && !alreadyHadExhaustInHand) {
    showStatEffect(state, target, -1, 'ATT');
    showStatEffect(state, target, -1, 'DEF');
  }
  if (destination === 'hand' && cardId === 'blessing-swiftness') showStatEffect(state, target, 1, 'MOV');
  if (destination === 'hand' && (cardId === 'banner' || cardId === 'banner-draw')) showStatEffect(state, target, 1, 'MOV');
  if (cardId === 'panic') target.panicAnimationSourceIds = [...new Set([...(target.panicAnimationSourceIds ?? []), sourceId])];
  return true;
}

function absorbBlessingShieldDamage(state: GameState, target: PlayerState, amount: number, sourceId: PlayerId, combatDamage = false): number {
  const pending = state.pendingAttack;
  const shieldPlayers = pending?.blessingShieldPlayerIds ?? (pending?.blessingShieldPlayerId ? [pending.blessingShieldPlayerId] : []);
  if (amount <= 0 || !pending?.blessingShieldApplied || !shieldPlayers.includes(target.id) || sourceId === target.id) return amount;
  ...
  // Ordinary combat prevention is recorded with the final combat total below.
  if (!combatDamage) recordCombatDamageBlocked(state, target, 1);
  state.log.unshift(`Blessing: Shield absorbed 1 ${combatDamage ? 'combat' : 'Attack / Defend Card effect'} Damage for ${target.name}.`);
  return amount - 1;
}
...
function dealCombatCardEffectDamage(state: GameState, target: PlayerState, amount: number, sourceId: PlayerId, sourceKind: 'attack' | 'defense', collision = false): number {
  ...
  if (adjusted > 0 && pending?.mythrilHelmetApplied && (target.id === pending.attackerId || target.id === pending.defenderId)) {
    recordCombatDamageBlocked(state, target, adjusted);
    state.log.unshift(`Mythril Helmet negated ${adjusted} ${sourceKind === 'attack' ? 'Attack' : 'Defend'} Card effect Damage to ${target.name}.`);
    adjusted = 0;
  }
  if (adjusted > 0 && pending?.blessingFaithApplied && (target.id === pending.attackerId || target.id === pending.defenderId)) {
    recordCombatDamageBlocked(state, target, adjusted);
    state.log.unshift(`Blessing: Faith negated ${adjusted} Damage to ${target.name}.`);
    adjusted = 0;
  }
  ...
  adjusted = absorbBlessingShieldDamage(state, target, adjusted, sourceId);
  if (adjusted > 0 && pending?.redirect && !pending.redirect.effectDamageRedirected && target.id === pending.defenderId && sourceId === pending.attackerId && sourceKind === 'attack') {
    const object = nextRedirectObject(state, target);
    if (object) {
      adjusted -= 1;
      pending.redirect.effectDamageRedirected = true;
      damageRedirectObject(state, object, sourceId, 'effect Damage');
...
function enterSpiritForm(state: GameState, player: PlayerState, reason: string) {
  ...
  if (!wasActive) {
    ...
    player.spiritMovementDepleted = false;
    player.deck = player.deck.filter((card) => card.cardId !== 'judgement');
    player.discard = player.discard.filter((card) => card.cardId !== 'judgement');
    if (!player.hand.some((card) => card.cardId === 'judgement')) {
      player.hand.push({ instanceId: `${player.id}-judgement-${++instanceSequence}`, cardId: 'judgement' });
      state.log.unshift(`${player.name} entered Spirit Form and added Judgement to their Hand.`);
    }
  }
...
function addBlessingCardToJohn(state: GameState, player: PlayerState, cardId: CardTypeId, source: BlessingAnimationSource, playAttackFirst = source === 'attack') {
  const card = cardDefinition({ instanceId: '', cardId });
  if (player.character !== 'john-christ' || !/\bBlessing\b/i.test(card.name)) return false;
  const cardInstanceId = `${player.id}-blessing-${++instanceSequence}`;
  const revealStoicShell = !player.traitBlocked && !player.stoicShell;
  player.hand.push({ instanceId: cardInstanceId, cardId, revealedToOpponent: true });
  if (!player.traitBlocked) player.stoicShell = true;
  state.blessingAnimations ??= [];
  state.blessingAnimations.push({ id: `${state.turn}-blessing-animation-${player.id}-${++instanceSequence}`, playerId: player.id, cardId, cardInstanceId, source, playAttackFirst, revealStoicShell });
  state.log.unshift(player.traitBlocked ? `${player.name} received ${card.name}, but Curse blocked Possessed from granting Stoic Shell.` : `${player.name} received ${card.name} and gained Stoic Shell.`);
  return true;
}

function hasCancelablePerkUse(state: GameState, playerId: PlayerId): boolean {
  const extended = state as GameState & WrecknaChoiceState & SpectreTargetingState & {
    fireball?: { casterId: PlayerId; undo: PerkTargetingUndo | null } | null;
    portal?: { casterId: PlayerId; undo: PerkTargetingUndo | null } | null;
    spiritGuardian?: { casterId: PlayerId; undo: PerkTargetingUndo | null } | null;
  };
  const pending = state.forceThrow ?? state.forcePull ?? state.arkaneArow ?? state.armDaWiz ?? state.preparation
    ?? state.arcaneMissle ?? state.chainLightning ?? state.magicHand ?? state.shizzle ?? state.mindTricks
    ?? extended.fireball ?? extended.portal ?? extended.spiritGuardian ?? extended.testPhylactery
    ?? extended.spectreReplicaPlacement ?? extended.spectrePerkOrigin ?? extended.spectreShadow
    ?? extended.lichdom ?? extended.dakkoth ?? extended.sap ?? extended.necronomicon ?? extended.decay;
  if (!pending || pending.casterId !== playerId || !pending.undo) return false;
  if (state.phase === 'choosing-mind-tricks-discard') return state.mindTricks?.discarded === 0;
  if (state.phase === 'shizzle-move') return state.shizzle?.started === false;
  return state.phase === 'choosing-force-throw-target'
    || state.phase === 'choosing-force-throw-direction'
    ...
    || state.phase === 'choosing-kyk-direction'
    ...
    || state.phase === 'choosing-shizzle-destination'
    ...
    || (state.phase as string) === 'choosing-dakkoth-tomb-square'
    || (state.phase as string) === 'choosing-sap-target'
    || (state.phase as string) === 'choosing-necronomicon-tomb'
    || (state.phase as string) === 'choosing-decay-target';
}
...
export function perkUseEventForTransition(before: GameState, command: GameCommand, after: GameState): PerkUseEvent | null {
  ...
  if (started) {
    if (hasCancelablePerkUse(after, started.playerId)) {
      after.pendingPerkUseEvent = started;
      return null;
    }
    after.pendingPerkUseEvent = null;
    return started;
  }
  const pending = before.pendingPerkUseEvent;
  if (!pending) return null;
  if (command.type === 'cancel-targeting') {
    after.pendingPerkUseEvent = null;
    return null;
  }
  if (hasCancelablePerkUse(after, pending.playerId)) {
    after.pendingPerkUseEvent = pending;
    return null;
  }
  after.pendingPerkUseEvent = null;
  return pending;
...
function soulStrikeForcedCards(player: PlayerState, kind: 'attack' | 'defend'): CardInstance[] {
  return player.hand.filter((card) => card.soulStrikeForcedUse === kind);
}
...
function resolveSoulStrikeAfterDefenseChosen(state: GameState, pending: PendingAttack, command: Extract<GameCommand, { type: 'defend' | 'pass-defense' }>) {
  ...
  if (combatDefenderProtectedFromNegativeEffects(state, defender)) {
    pending.soulStrikeResult = { outcome: 'prevented' };
    state.log.unshift(`${combatProtectionName(state)} prevented Soul Strike's additional effect.`);
    return;
  }
  const selectedDefenseId = command.type === 'defend' ? command.cardInstanceId : null;
  const remainingHand = defender.hand.filter((card) => card.instanceId !== selectedDefenseId);
  if (remainingHand.length === 0) {
    const dealt = dealDamage(state, defender, 2, false, attacker.id, 'attack');
    pending.soulStrikeResult = { outcome: 'damage', damage: dealt };
    state.log.unshift(`Soul Strike dealt ${dealt} additional Damage because ${defender.name} had no Cards left in Hand after choosing whether to Block.`);
    return;
  }
  const candidates = remainingHand.filter((card) => {
    const kind = cardDefinition(card).kind;
    return kind === 'perk' || kind === 'attack' || kind === 'defend';
  });
  if (candidates.length === 0) {
    pending.soulStrikeResult = { outcome: 'no-eligible' };
  ...
  if (kind === 'perk') {
    const name = cardDefinition(revealed).name;
    pending.soulStrikeResult = { cardId: revealed.cardId, outcome: 'discarded-perk' };
    discardFromHand(defender, revealed.instanceId);
    state.log.unshift(`Soul Strike privately revealed ${name} to Spectre and forced ${defender.name} to discard it.`);
    return;
  }
  if (kind !== 'attack' && kind !== 'defend') return;
  revealed.soulStrikeForcedUse = kind;
  pending.soulStrikeResult = { cardId: revealed.cardId, outcome: kind === 'attack' ? 'forced-attack' : 'forced-defend' };
  state.log.unshift(`Soul Strike privately revealed and marked 1 ${kind === 'attack' ? 'Attack' : 'Block'} Card in ${defender.name}'s Hand; that Card must be used first.`);
}
...
function resolveSpectreAttack(state: GameState, command: Extract<GameCommand, { type: 'spectre-attack' }>): CommandResult {
  ...
  if (attacker.wrecknaInsideTombId && state.objects.some((object) => object.id === attacker.wrecknaInsideTombId && object.kind === 'tomb')) return fail(state, 'Wreckna cannot use Attack Cards while inside a Tomb.');
  ...
  const forcedAttacks = soulStrikeForcedCards(attacker, 'attack');
  if (forcedAttacks.length > 0 && !forcedAttacks.some((card) => card.instanceId === instance.instanceId)) return fail(state, 'Soul Strike requires a marked Attack Card to be used first.');
  const replicas = spectreReplicas(state, attacker.id);
  if (command.origin === 'replica' && attacker.traitBlocked) return fail(state, 'Curse prevents Spectre from attacking through a replica.');
  if (command.origin === 'replica' && (attacker.character !== 'spectre' || replicas.length === 0)) return fail(state, 'Only Spectre can Attack from her replica.');
  ...
  const reachableReplica = target ? replicas.find((candidate) => distance(candidate.position, target) <= 1 && hasLineOfSight(state, candidate.position, target) && canAttackTargetSquare(state, candidate.position, target)) : undefined;
  const effectiveOrigin: 'spectre' | 'replica' = command.origin === 'replica' && !spectreCanReachTarget ? 'replica' : 'spectre';
  if (effectiveOrigin === 'replica' && !reachableReplica) return fail(state, 'The target is outside every replica’s Attack Range.');
  const origin = effectiveOrigin === 'replica' ? reachableReplica!.position : attacker.position;
  ...
  const card = cardDefinition(instance);
  const attackingFromBox = effectiveOrigin === 'spectre' ? Boolean(attacker.spectreOnBoxId) : Boolean(reachableReplica?.spectreOnBoxId);
  ...
  const baseHighGroundBonus = Number(boxElevationAdvantage || (isHighGround(state, origin) || attackingFromBox) && isLowGroundOrProtected(state, combatTarget));
  const highGroundBonus = baseHighGroundBonus * (card.id === 'lightbringer' ? 2 : 1);
  const exhaustPenalty = Number(attacker.hand.some((entry) => entry.cardId === 'exhaust'));
  ...
  const shadowTransitObjectIds = new Set(isSpectreShadowTrailCell(state, attacker, attacker.position)
    ...
    : []);
  const solitudeEligible = card.id === 'solitude' && !Object.values(state.players).some((player) => player.id !== attacker.id && player.hp > 0 && player.id !== defender.id && distance(player.position, combatTarget) === 1)
    && !state.objects.some((object) => object.id !== targetReplica?.id && !(object.kind === 'spectre-replica' && object.ownerId === attacker.id) && !shadowTransitObjectIds.has(object.id) && distance(object.position, combatTarget) === 1);
  const lightsaberBonus = attacker.character === 'shinobi' && attacker.lightsaberBuff && !attacker.traitBlocked ? 1 : 0;
  const rageBonus = attacker.character === 'orkk' && !attacker.traitBlocked ? attacker.rageStacks : 0;
  const magicianBonus = attacker.character === 'magician' ? attacker.arcaneBoltAttackBonus : 0;
  const spiritBonus = attacker.character === 'john-christ' && attacker.spiritForm && !attacker.traitBlocked ? 2 : 0;
  ...
  if (card.id === 'mana-barrage' && attacker.manaMode === 'consume') {
...
function automaticBannerDiscardCommand(result: CommandResult): GameCommand | null {
  ...
  else if (state.phase === 'choosing-blink-discard') { playerId = state.pendingAttack?.defenderId; type = 'blink-discard'; }
  else if (state.phase === 'choosing-preparation-discard') { playerId = state.preparation?.casterId; type = 'preparation-discard'; }
  else if (state.phase === 'choosing-snowball-discard') { playerId = state.activePlayerId; type = 'snowball-discard'; }
  else if (state.phase === 'mana-blast-offer') { playerId = state.pendingAttack?.defenderId; type = 'mana-blast-discard'; }
  else if (state.phase === 'choosing-grimoire-discard') { playerId = state.pendingAttack?.defenderId; type = 'grimoire-discard'; }
  else if (state.phase === 'choosing-shadow-barter-discard') { playerId = (state as ShadowBarterState).shadowBarter?.defenderId; type = 'shadow-barter-discard'; }
  else if (state.phase === 'choosing-mind-tricks-enemy-discard') { playerId = state.mindTricks?.enemyId; type = 'mind-tricks-enemy-discard'; }
  else if (['choosing-guard-discard', 'choosing-dash-discard', 'choosing-hot-potato-discard'].includes(state.phase as string) || (state.phase === 'choosing-end-discard' && state.players[state.activePlayerId].hand.length > 5)) { playerId = state.activePlayerId; type = 'discard-card'; }
  if (!playerId || !type) return null;
  const banner = state.players[playerId]?.hand.find((card) => card.cardId === 'banner' || card.cardId === 'banner-draw');
  return banner ? ({ type, playerId, cardInstanceId: banner.instanceId } as GameCommand) : null;
}
...
function applyCommandInternal(source: GameState, rawCommand: unknown): CommandResult {
  ...
  if (command.type === 'fireball-target') return resolveFireballTarget(state, command.playerId, command.targetId, command.targetKind);
  if (command.type === 'portal-teleport') return resolvePortalTeleport(state, command.playerId, command.to);
  if (command.type === 'sweet-potato-choice') return resolveSweetPotatoChoice(state, command.playerId, command.choice);
  if (command.type === 'ack-combat') return acknowledgeCombat(state, command.playerId, command.combatExpiresAt);
  if (command.type === 'oracle-reveal') return resolveOracleReveal(state, command.playerId);
  if (command.type === 'yamato-move') return resolveYamatoMove(state, command.playerId, command.to);
  ...
  if (command.type === 'frostmourne-decision') return resolveFrostmourneDecision(state, command.playerId, command.use);
  if (command.type === 'lightbringer-swap-decision') return resolveLightbringerSwapDecision(state, command.playerId, command.swap);
  ...
  if (command.type === 'spectre-perk-origin-select') return selectSpectrePerkOrigin(state, command.playerId, command.origin, command.replicaId);
  ...
  if (command.type === 'boomerang-target') return resolveBoomerangTarget(state, command.playerId, command.targetId, command.targetKind);
  ...
  if (command.type === 'move') {
    ...
    const freeTombTransfer = player.character === 'wreckna' && !player.traitBlocked && Boolean(currentTomb) && Boolean(destinationTomb) && currentTomb!.id !== destinationTomb!.id
    ...
    const enteringTomb = player.character === 'wreckna' && !player.traitBlocked && Boolean(destinationTomb) && player.wrecknaInsideTombId !== destinationTomb!.id;
    ...
    const spiritTraversalActive = player.spiritForm && !player.traitBlocked;
    const spiritEnemySquares = spiritTraversalActive
      ...
      : 0;
    const spiritObjectSquares = spiritTraversalActive
    ...
    if (player.character === 'merylin') player.movementSpentThisTurn = (player.movementSpentThisTurn ?? 0) + cost;
    if (player.character === 'john-christ') {
      if (spiritTraversalActive) {
  ...
  if (command.type === 'free-move') {
    return resolveFreeMove(state, player);
  }
  if (command.type === 'attack') {
    ...
    if (player.wrecknaInsideTombId && state.objects.some((object) => object.id === player.wrecknaInsideTombId && object.kind === 'tomb')) return fail(source, 'Wreckna cannot use Attack Cards while inside a Tomb.');
    if (!hasAttackAction(player)) return fail(source, 'No Attack actions remain.');
    const instance = player.hand.find((card) => card.instanceId === command.cardInstanceId);
    if (!instance || cardDefinition(instance).kind !== 'attack') return fail(source, 'That Attack card is not in the hand.');
    const forcedAttacks = soulStrikeForcedCards(player, 'attack');
    if (forcedAttacks.length > 0 && !forcedAttacks.some((card) => card.instanceId === instance.instanceId)) return fail(source, 'Soul Strike requires a marked Attack Card to be used first.');
    const card = cardDefinition(instance);
    ...
    if (!attackCardTargetInRange(state, player, card.id, defender.position)) {
      return fail(source, card.id === 'excalibur'
        ...
        : `Target is outside ${card.name}'s attack range.`);
    }
    ...
    if (player.character === 'merylin' && (!player.merylinSummonActive || player.traitBlocked)) return fail(source, player.traitBlocked ? 'Curse blocks Swordcraft, so Summon cannot enable Merylin\'s Attack Cards.' : 'Merylin requires Summon to use an Attack Card.');
    consumeMerylinSummonForAttack(state, player);
    ...
    const lightsaberBonus = player.character === 'shinobi' && player.lightsaberBuff && !player.traitBlocked ? 1 : 0;
    const rageBonus = player.character === 'orkk' && !player.traitBlocked ? player.rageStacks : 0;
    const baseHighGroundBonus = highGroundAttackValueBonus(state, player, defender.position);
    const highGroundBonus = baseHighGroundBonus * (card.id === 'lightbringer' ? 2 : 1);
    const exhaustPenalty = Number(player.hand.some((entry) => entry.cardId === 'exhaust'));
  ...
  if (command.type === 'guard') {
    ...
    if (!player.hand.some((entry) => !cardDefinition(entry).cannotBeDiscarded && entry.cardId !== 'judgement')) {
  ...
  if (command.type === 'dash') {
    ...
    if (!player.hand.some((entry) => !cardDefinition(entry).cannotBeDiscarded && !isBlessingCard(entry) && entry.cardId !== 'judgement')) return fail(source, 'There is no eligible non-Blessing Card available to discard for Dash.');
    state.dashCancellation = { previousMovementRemaining: player.movementRemaining, discardedCard: null };
    (state.dashCancellation as typeof state.dashCancellation & { previousJohnCumulativeMovementRemaining: number }).previousJohnCumulativeMovementRemaining = player.johnCumulativeMovementRemaining;
...
function playPerkFromHand(state: GameState, player: PlayerState, command: Extract<GameCommand, { type: 'play-perk' }>): CommandResult {
  ...
  if (perk.id === 'fireball' || perk.id === 'sweet-potato') {
    ...
    if (player.actionsRemaining <= 0) return fail(state, `${perk.name} requires 1 Action.`);
    if (command.destination !== 'direct') return fail(state, `${perk.name} is an Action Reward Card and cannot be placed in Spell Echo.`);
    const targetingUndo = snapshotPerkTargeting(player);
    player.actionsRemaining -= 1;
    removeCard(player, instance.instanceId);
    if (perk.id === 'fireball') {
      ...
      (state as GameState & { sweetPotato?: { casterId: PlayerId; undo: PerkTargetingUndo } | null }).sweetPotato = { casterId: player.id, undo: targetingUndo };
      state.phase = 'choosing-sweet-potato' as GamePhase;
      state.log.unshift(`${player.name} used Sweet Potato as an Action and must choose healing or Status removal.`);
    }
    return ok(state);
  }
  const preliminaryValidation = validatePerkAction(state, player, true);
  if (preliminaryValidation) return preliminaryValidation;
  if (!player.freeMoveUsed && perkGrantsMovement(perk.id, 1)) resolveFreeMove(state, player);
  ...
  if (perk.id === 'vicious-mockery' || perk.id === 'vicious-mockery-1') return fail(state, 'Vicious Mockery can only be applied optionally during combat.');
  ...
  const targetingUndo = snapshotPerkTargeting(player);
  player.actionsRemaining -= 1;
  if (player.perkUsed) player.spellsingerExtraPerkUses = Math.max(0, (player.spellsingerExtraPerkUses ?? 0) - 1);
  ...
  if (oldPositionOne) {
    ...
    if (!oldPositionOne.oneTimeCopy) player.discard.push(prepareCardForDiscard(oldPositionOne));
  }
...
function validatePerkAction(state: GameState, player: PlayerState, ignorePanic = false): CommandResult | null {
  ...
  if (player.perkUsed && (player.spellsingerExtraPerkUses ?? 0) <= 0) return fail(state, 'Only one Perk action may be used per turn.');
  if (!ignorePanic && player.hand.some((card) => card.cardId === 'panic')) return fail(state, 'Panic prevents the use of Perk Cards until Free Move Removes it.');
  return null;
}

function perkGrantsMovement(perkId: CardTypeId, level: number): boolean {
  return perkId === 'windwalker-stance'
    || perkId === 'swiftform'
    || perkId === 'shadow-dagger'
    || perkId === 'relocate'
    || (perkId === 'blessed-prayer' && level >= 2)
    || perkId === 'decay'
    || (perkId === 'dakkoth' && level >= 3)
    || (perkId === 'barbarian-stance' && level >= 3);
}

function resolveFreeMove(state: GameState, player: PlayerState): CommandResult {
  if (player.freeMoveUsed) return fail(state, 'Free Move was already used this turn.');
...
function applyPerkEffects(state: GameState, player: PlayerState, perk: Card, level: number) {
  if (perk.id === 'monarch-flush-perk') {
    for (const opponent of Object.values(state.players)) {
      if (opponent.id === player.id) continue;
      opponent.hand.forEach((instance) => { instance.revealedToOpponent = true; });
    }
    state.log.unshift(`${player.name} used Monarch's Flush as a Perk, revealed every opponent Hand, and Removed Monarch's Flush from the game.`);
    return;
  }
  if (perk.id === 'firebolt') {
  ...
  if (perk.id === 'spellsinger-stance') {
    ...
    player.knownTopCardId = player.knownTopCardIds[0] ?? null;
    player.spellsingerExtraPerkUses = Math.max(player.spellsingerExtraPerkUses ?? 0, 1);
    if (level >= 2) grantMerylinSummon(state, player.id, 'Spellsinger Stance');
    if (level >= 3) player.spellsingerExtraAttacks = (player.spellsingerExtraAttacks ?? 0) + 1;
    state.log.unshift(`Spellsinger Stance level ${level}: ${player.name} privately revealed ${revealCount} top Deck Card${revealCount === 1 ? '' : 's'} and may use another Perk this turn${level >= 2 ? ', then gained Summon' : ''}${level >= 3 ? ' and 1 extra Attack' : ''}.`);
    return;
  }
  ...
  if (perk.id === 'barbarian-stance') {
    const movementBefore = player.movementRemaining;
    player.movementRemaining = Math.max(0, player.movementRemaining - 1);
    const attackBonus = level >= 2 ? 2 : 1;
    const previousAttackBonus = player.barbarianNextAttackBonus ?? 0;
    player.barbarianNextAttackBonus = Math.max(player.barbarianNextAttackBonus ?? 0, attackBonus);
    showStatEffect(state, player, player.movementRemaining - movementBefore, 'MOV');
  ...
  if (perk.id === 'carian-stance') {
    const drawn = drawCards(player, 1);
    grantMerylinSummon(state, player.id, 'Carian Stance');
    if (level >= 2) {
      const gainedDefense = Math.max(0, 1 - (player.merylinSummonedDefenseBonus ?? 0));
      player.merylinSummonedDefenseBonus = 1;
      showStatEffect(state, player, gainedDefense, 'DEF');
    }
    if (level >= 3) player.carianReturnNextDefend = true;
    state.log.unshift(`Carian Stance level ${level}: ${player.name} drew ${drawn} Card${drawn === 1 ? '' : 's'} and gained Summon${level >= 2 ? ' with +1 DEF while Summoned' : ''}${level >= 3 ? '; the next Defend Card played will return to Hand' : ''}.`);
    return;
  }
  ...
  if (perk.id === 'shadow-dagger') {
    ...
    if (!replica) {
      ...
      (state as SpectreTargetingState).spectrePerkOrigin = { casterId: player.id, perkId: 'shadow-dagger', level, origin: 'spectre', replicaId: null, undo: null };
      state.phase = 'choosing-spectre-perk-origin';
  ...
  if (perk.id === 'consume-replica') {
    const replicas = spectreReplicas(state, player.id);
    if (replicas.length === 0) { state.log.unshift('Shadow Cloak found no replica and had no effect.'); return; }
    for (const replica of replicas) state.objectPushAnimations.push({
      id: `${state.turn}-shadow-cloak-${replica.id}-${++instanceSequence}`,
      objectId: replica.id,
      from: { ...replica.position }, to: { ...replica.position }, dx: 0, dy: 0,
      collided: false, removeOnComplete: true, destroy: true, shadowDissolve: true,
    });
    state.objects = state.objects.filter((object) => !(object.kind === 'spectre-replica' && object.ownerId === player.id));
    if (player.spectreRelocateTetherReplicaId && replicas.some((replica) => replica.id === player.spectreRelocateTetherReplicaId)) player.spectreRelocateTetherReplicaId = null;
    addForcedStatusCard(state, player, 'headache', 'hand', player.id, 'perk', true);
    const removedEffects = level >= 2 ? removeNegativeHandEffects(player) : 0;
    if (level >= 3 && removedEffects > 0) {
      player.spectreAttackBonus = (player.spectreAttackBonus ?? 0) + removedEffects;
      showStatEffect(state, player, removedEffects, 'ATT');
    }
    player.spectreShadowCloakActive = true;
    state.log.unshift(`Shadow Cloak dissolved ${replicas.length} replica${replicas.length === 1 ? '' : 's'}, granted Spell Immunity until the beginning of ${player.name}'s next turn, and added Headache${level >= 2 ? `; removed ${removedEffects} negative Hand effect${removedEffects === 1 ? '' : 's'}` : ''}${level >= 3 ? ` and granted +${removedEffects} ATT until turn end` : ''}.`);
    return;
  }
  if (perk.id === 'haunt') {
    resolveSpectreHaunt(state, player, level);
    return;
  }
  if (perk.id === 'decay') {
    ...
    state.log.unshift(`Curse level ${level}: choose an enemy within Range ${effectiveAttackRange(state, player)} and line of sight.`);
    return;
  }
  ...
  if (perk.id === 'sap') {
    const range = effectiveAttackRange(state, player) + (level >= 2 ? 2 : 0);
    (state as WrecknaChoiceState).sap = { casterId: player.id, level, range, undo: null };
    state.phase = 'choosing-sap-target' as GamePhase;
    state.log.unshift(`Sap level ${level}: choose an enemy within Range ${range} and line of sight.`);
    return;
  }
  if (perk.id === 'dakkoth') {
  ...
  if (perk.id === 'fear-the-justice') {
    ...
    const affectedEnemies = level >= 2
      ? enemyBodies(state, player.id).filter((body) => !state.players[body.ownerId].spectreShadowCloakActive && distance(player.position, body.position) === 1)
      : [];
    for (const enemy of affectedEnemies) {
      addForcedStatusCard(state, state.players[enemy.ownerId], 'panic', 'hand', player.id, 'perk', true);
      state.log.unshift(`Fear the Justice applied Panic to adjacent enemy ${enemy.name}.`);
    }
    if (level >= 3) {
      const discardTargets = affectedEnemies.filter((enemy) => state.players[enemy.ownerId].hand.some((card) => cardDefinition(card).kind === 'defend')).map((enemy) => enemy.ownerId);
  ...
  if (perk.id === 'blessed-prayer') {
    addBlessingCardToJohn(state, player, 'blessing-prayer', 'perk');
    if (level >= 2) {
  ...
  if (perk.id === 'chain-lightning') {
    const baseBounces = level >= 3 ? 2 : 1;
    state.chainLightning = { casterId: player.id, level, bounces: baseBounces + (player.manaMode === 'consume' ? 3 : 0), bounceRange: level >= 2 ? 2 : 1, undo: null };
    state.phase = 'choosing-chain-lightning-target';
  ...
  if (perk.id === 'higround-advantage') {
    ...
    if (level >= 2 && player.character === 'shinobi') {
      ...
      const newlyActive = !player.lightsaberBuff && !player.traitBlocked;
      player.lightsaberBuff = true; player.lightsaberStacks += 1; player.lightsaberAppliedWhileTraitBlocked = Boolean(player.traitBlocked);
      adjustUnspentMovementForRangeChange(player, previousMoveRange);
      if (newlyActive) showLightsaberStats(state, player);
  ...
  if (perk.id === 'force-pull') {
    state.forcePull = { casterId: player.id, level, distance: level >= 2 ? 2 : 1, targetRange: level >= 2 ? 4 : 3, undo: null };
    state.phase = 'choosing-force-pull-target';
  ...
  if (perk.id === 'consume-rage') {
    ...
    if (level >= 3) {
      const adjacentEnemies = enemyBodies(state, player.id).filter((body) => !state.players[body.ownerId].spectreShadowCloakActive && distance(body.position, player.position) === 1);
      adjacentEnemies.forEach((enemy) => addForcedStatusCard(state, state.players[enemy.ownerId], 'exhaust', 'hand', player.id, 'perk', true));
  ...
  if (perk.id === 'swiftform') {
    ...
    const gainedMovement = Math.max(0, moveBonus - (player.swiftformMoveBonus ?? 0));
  ...
  if (perk.id === 'mind-tricks') {
    ...
    if (state.players[enemyId]?.spectreShadowCloakActive) {
      state.log.unshift(`Shadow Cloak made ${state.players[enemyId].name} ineligible for Mind Tricks.`);
      return;
    }
    state.mindTricks = { casterId: player.id, level, maxDiscards: level >= 3 ? 2 : 1, discarded: 0, revealedInstanceIds: [], enemyId, enemyDiscardsRemaining: 0, undo: null };
    state.phase = 'choosing-mind-tricks-discard';
    state.log.unshift(`Mind Tricks level ${level}: ${player.name} may reveal up to ${level >= 3 ? 2 : 1} card${level >= 3 ? 's' : ''}, or resolve it without revealing.`);
  }
}
...
function completeInnerPeace(state: GameState, player: PlayerState, level: number) {
  ...
  if (level >= 3) {
    addBlessingCardToJohn(state, player, 'blessing-faith', 'perk');
...
function resolveViciousMockeryDecision(state: GameState, playerId: PlayerId, use: boolean): CommandResult {
  ...
  if (use) {
    const player = state.players[playerId]; const index = player.hand.findIndex((card) => card.cardId === 'vicious-mockery') >= 0 ? player.hand.findIndex((card) => card.cardId === 'vicious-mockery') : player.hand.findIndex((card) => card.cardId === 'vicious-mockery-1');
    if (index < 0) return fail(state, 'Vicious Mockery is not in this Player’s Hand.');
    const [mockery] = player.hand.splice(index, 1); const bonus = cardDefinition(mockery).value; choice.applied.push(playerId); choice.appliedValues[playerId] = bonus;
    if (state.pendingAttack?.attackerId === playerId) {
      state.pendingAttack.attackValue += bonus;
      state.combatReveal!.attackTotal += bonus;
    } else state.combatReveal!.defendTotal += bonus;
    state.log.unshift(`${player.name} applied ${cardDefinition(mockery).name} for +${bonus} ${state.pendingAttack?.attackerId === playerId ? 'ATT' : 'DEF'} and Removed it from the game.`);
  }
  choice.decided.push(playerId);
  if (choice.decided.length < choice.eligible.length) return ok(state);
  const defenseCommand = choice.defenseCommand; const defenderMockery = choice.appliedValues[defenseCommand.playerId] ?? 0;
...
function resolveExhaustDecision(state: GameState, playerId: PlayerId, use: boolean): CommandResult {
  ...
  if (use) {
    ...
    if (state.pendingAttack?.attackerId === playerId) {
      const stillExhausted = player.hand.some((card) => card.cardId === 'exhaust');
      const additionalPenalty = stillExhausted ? 2 : 1;
      state.pendingAttack.attackValue -= additionalPenalty;
      state.pendingAttack.attackModifiers = [
        ...(state.pendingAttack.attackModifiers ?? []).filter((modifier) => stillExhausted || !modifier.source.includes('Exhaust Cards in Hand')),
        { value: -2, source: 'attached Exhaust' },
      ];
      if (state.combatReveal) state.combatReveal.attackTotal -= additionalPenalty;
    }
    state.log.unshift(`${player.name} attached and Removed Exhaust for a -2 card Value modifier.`);
  }
...
function resolveBlessingFaithDecision(state: GameState, playerId: PlayerId, use: boolean): CommandResult {
  ...
  const player = state.players[playerId];
  if (playerId === pending.attackerId ? pending.attackerWasInSpiritForm : player.spiritForm) return fail(state, 'Blessing: Faith cannot be used while John is in Spirit Form.');
...
function resolveFeedSpiritDecision(state: GameState, playerId: PlayerId, cardInstanceId: string | null): CommandResult {
  ...
  if (cardInstanceId) {
    ...
    const requestedHealing = pending.feedSpiritCombatDamage ?? 0;
    const healed = healPlayer(state, john, requestedHealing);
    state.log.unshift(`Feed the Spirit Removed ${cardDefinition(blessing).name} and restored ${healed} Hit Point${healed === 1 ? '' : 's'} to ${john.name}, matching the ${requestedHealing} HP lost from combat Damage.`);
...
function graveyardDefenseBonus(pending: PendingAttack, defenseCardId: CardTypeId | null): number {
...
function beginMultiplayerCombatStack(state: GameState, command: Extract<GameCommand, { type: 'defend' | 'pass-defense' }>): CommandResult {
  ...
  const revealedDefendTotal = defendDefinition
    ? (defendDefinition.id === 'mana-baryer' && defender.shieldEquipped ? 5 : defendDefinition.value
      ...
      + (defender.character === 'shinobi' && defender.lightsaberBuff && !defender.traitBlocked ? 1 : 0)
      + (defender.character === 'orkk' && defender.shieldEquipped ? 1 : 0))
      + graveyardDefenseBonus(pending, defendDefinition.id)
...
export function resolveMultiplayerCombatStack(state: GameState, selections: Partial<Record<PlayerId, string[]>>): CommandResult {
  ...
  for (const playerId of [pending.attackerId, pending.defenderId]) {
    ...
    for (const card of selectedCards as CardInstance[]) {
      ...
      if (card.cardId === 'vicious-mockery' || card.cardId === 'vicious-mockery-1') {
        ...
        if (playerId === pending.attackerId) { pending.attackValue += bonus; pending.attackModifiers = [...(pending.attackModifiers ?? []), { value: bonus, source: cardDefinition(card).name }]; }
        else defenderMockery = bonus;
      ...
      } else if (card.cardId === 'helmet') {
        pending.combatStackDefenderHelmet = true;
      } else if (card.cardId === 'exhaust') {
        if (playerId === pending.attackerId) {
          const stillExhausted = player.hand.some((heldCard) => heldCard.cardId === 'exhaust');
          pending.attackValue -= stillExhausted ? 2 : 1;
          pending.attackModifiers = [
            ...(pending.attackModifiers ?? []).filter((modifier) => stillExhausted || !modifier.source.includes('Exhaust Cards in Hand')),
            { value: -2, source: 'attached Exhaust' },
          ];
        }
  ...
  if (pending.blessingLightApplied && combatDefenderProtectedFromNegativeEffects(state, state.players[pending.defenderId])) {
    pending.blessingLightApplied = false;
    state.log.unshift(`${combatProtectionName(state)} prevented Blessing: Light from reducing the defender's Block Value.`);
  }
...
function resolveOrderedPreCombat(state: GameState, command: Extract<GameCommand, { type: 'defend' | 'pass-defense' }>): CommandResult | null {
  ...
  if (attackEffectsCancelled) {
    if (pending.cardId === 'bone-chill') {
      const meleeBonus = pending.attackModifiers?.find((modifier) => modifier.source === 'Bone Chill · melee');
      if (meleeBonus) {
        pending.attackValue -= meleeBonus.value;
        pending.attackModifiers = pending.attackModifiers?.filter((modifier) => modifier !== meleeBonus);
      }
    }
  ...
  if (pending.cardId === 'drain-strength') {
    ...
    pending.attackPreCombatResolved = true;
    if (!combatDefenderProtectedFromNegativeEffects(state, defender) && available.length > 0) {
    ...
    if (available.length === 0) {
      ...
      if (!combatDefenderProtectedFromNegativeEffects(state, defender)) {
        ...
        showStatEffect(state, defender, -2, 'MOV');
        showStatEffect(state, attacker, 2, 'MOV');
...
function applyHexBeforeCombat(state: GameState, command: Extract<GameCommand, { type: 'defend' | 'pass-defense' }>) {
  ...
  const defenseCardId = command.type === 'defend'
    ...
    : null;
  if (defenseCardId && ['block', 'da-blokk', 'spellblock', 'blessed-block', 'tomb-block', 'decisive-block'].includes(defenseCardId)) {
  ...
  if (combatDefenderProtectedFromNegativeEffects(state, target)) {
    state.log.unshift(`${combatProtectionName(state)} prevented Hex's movement steal.`);
    return;
  }
  stealWrecknaMovement(state, attacker, target, 1, 'Hex');
}
...
function stealWrecknaMovement(state: GameState, attacker: PlayerState, target: PlayerState, amount: number, source: 'Hex' | 'Bone Chill') {
  ...
  target.hexMovementStolenBy[attacker.id] = (target.hexMovementStolenBy[attacker.id] ?? 0) + amount;
  adjustUnspentMovementForRangeChange(target, previousTargetRange);
  attacker.hexMovementBonus = (attacker.hexMovementBonus ?? 0) + amount;
  grantMovement(attacker, amount);
  showStatEffect(state, target, -amount, 'MOV');
  showStatEffect(state, attacker, amount, 'MOV');
...
type YamatoChoiceState = GameState & { yamato?: { defenderId: PlayerId; defenseCommand: Extract<GameCommand, { type: 'defend' }> } | null };
type OraclePendingAttack = PendingAttack & { oracleInstanceId?: string; oracleValueAtCombatStart?: number; oracleRevealedThisCombat?: boolean; oracleAttackRevealed?: boolean };

function resolveOracleReveal(state: GameState, playerId: PlayerId): CommandResult {
  const pending = state.pendingAttack as OraclePendingAttack | null;
  if (state.phase !== 'defending' || !pending || pending.defenderId !== playerId || !pending.oracleInstanceId) return fail(state, 'Oracle cannot reveal an Attack Card now.');
  if (pending.oracleRevealedThisCombat) return fail(state, 'Oracle already revealed this Attack Card.');
  if (pending.oracleValueAtCombatStart === 1) return fail(state, 'Oracle began this combat at Value 1 and must be used to Defend.');
  const defender = state.players[playerId];
  const oracle = defender.hand.find((card) => card.instanceId === pending.oracleInstanceId && card.cardId === 'oracle');
  if (!oracle) return fail(state, 'Oracle is no longer in Hand.');
  const previousValue = cardBaseValue(oracle);
  oracle.oracleValue = Math.max(1, previousValue - 1);
  pending.oracleRevealedThisCombat = true;
  pending.oracleAttackRevealed = true;
  state.log.unshift(`${defender.name} used Oracle to reveal ${cardDefinition({ instanceId: '', cardId: pending.cardId }).name}; Oracle's base Defend Value decreased from ${previousValue} to ${oracle.oracleValue} and cannot Defend this Attack.`);
  return ok(state);
}

function yamatoAttackStillInRange(state: GameState, pending: PendingAttack, defenderPosition: Cell): boolean {
  const attacker = state.players[pending.attackerId];
  const origin = pending.attackerPosition ?? attacker.position;
  const attackDistance = distance(origin, defenderPosition);
  if (pending.cardId !== 'excalibur') return attackDistance <= effectiveAttackRange(state, attacker);
  const dx = Math.abs(defenderPosition.x - origin.x);
  const dy = Math.abs(defenderPosition.y - origin.y);
...
type BlinkCombatReveal = CombatReveal & { blinkTeleport?: { defenderId: PlayerId; from: Cell; to: Cell; missed: boolean } };
function blinkHandCostEligible(card: CardInstance, playedBlinkInstanceId: string): boolean {
  return card.instanceId !== playedBlinkInstanceId && card.cardId !== 'pinned' && !cardDefinition(card).cannotBeDiscarded;
}
...
function resolveYamatoMove(state: GameState, playerId: PlayerId, to: Cell | null): CommandResult {
  ...
  const pending = state.pendingAttack;
  if ((state.phase as string) !== 'choosing-yamato-move' || !choice || !pending || choice.defenderId !== playerId || pending.defenderId !== playerId) return fail(state, 'Yamato is not waiting for this movement choice.');
  const defender = state.players[playerId];
  if (to) {
    if (distance(defender.position, to) !== 1) return fail(state, 'Yamato may move exactly 1 adjacent Square.');
    if (to.x < 1 || to.x > boardWidth(state) || to.y < 0 || to.y >= boardHeight(state)) return fail(state, 'That Square is outside the Board.');
    if (Object.values(state.players).some((player) => player.id !== defender.id && player.hp > 0 && player.position.x === to.x && player.position.y === to.y) || state.objects.some((object) => object.position.x === to.x && object.position.y === to.y)) return fail(state, 'Yamato requires an empty destination Square.');
    if (diagonalMovementBlockedByObject(state, defender.position, to)) return fail(state, 'An Object blocks that diagonal movement.');
    if (isForbiddenSlideAscent(state, defender.position, to)) return fail(state, 'Yamato cannot move up that Slide edge.');
  ...
  } else state.log.unshift(`${defender.name} chose not to move with Yamato.`);
  (pending as PendingAttack & { yamatoMoveResolved?: boolean }).yamatoMoveResolved = true;
  extended.yamato = null;
  if (yamatoAttackStillInRange(state, pending, defender.position)) {
    state.phase = 'defending';
    return resolveDefense(state, choice.defenseCommand);
  }
...
type LightbringerPendingAttack = PendingAttack & {
  lightbringerSwapResolved?: boolean;
  lightbringerDefenseCommand?: Extract<GameCommand, { type: 'defend' | 'pass-defense' }>;
};
...
function resolveLightbringerSwapDecision(state: GameState, playerId: PlayerId, swap: boolean): CommandResult {
  ...
  const attacker = state.players[pending.attackerId];
  const defender = state.players[pending.defenderId];
  const swapPreventedByDefense = swap && combatDefenderProtectedFromNegativeEffects(state, defender);
  ...
  if (swap && !swapPreventedByDefense) {
    ...
    const newHighGround = highGroundAttackValueBonus(state, { ...attacker, position: pending.attackerPosition }, pending.defenderPosition) * 2;
    pending.attackValue += newHighGround - oldHighGround;
    pending.attackModifiers = [
      ...(pending.attackModifiers ?? []).filter((modifier) => !(modifier.source.includes('Lightbringer') && modifier.source.includes('High Ground'))),
      ...(newHighGround ? [{ value: newHighGround, source: 'Lightbringer · High Ground ×2' }] : []),
    ];
    state.log.unshift(`Lightbringer swapped ${attackerReplica ? `${attacker.name}'s attacking replica` : attacker.name} and ${defenderReplica ? `${defender.name}'s attacked replica` : defender.name} after the Defend Card was chosen.`);
  } else if (!swapPreventedByDefense) state.log.unshift(`${attacker.name} declined Lightbringer's position swap.`);
  pending.lightbringerSwapResolved = true;
  const defenseCommand = pending.lightbringerDefenseCommand;
  delete pending.lightbringerDefenseCommand;
  state.phase = 'defending';
  return resolveDefense(state, defenseCommand);
}
...
function resolveDefense(state: GameState, command: Extract<GameCommand, { type: 'defend' | 'pass-defense' }>, exhaustResolved = false, defenderAttachedExhaust = false, mockeryResolved = false, defenderMockery = 0): CommandResult {
  ...
  if (oraclePending.oracleValueAtCombatStart === 1) {
    if (command.type === 'pass-defense') return fail(state, 'Oracle began this combat at Value 1 and must be used to Defend.');
    if (command.cardInstanceId !== oraclePending.oracleInstanceId) return fail(state, 'Oracle began this combat at Value 1; no other Defend Card may be used.');
  }
  if (command.type === 'defend' && command.cardInstanceId === oraclePending.oracleInstanceId && oraclePending.oracleRevealedThisCombat) return fail(state, 'Oracle revealed this Attack Card and cannot Defend against it.');
  if (command.type === 'defend') {
  ...
  if (command.type === 'defend') {
    const selectedDefense = defender.hand.find((card) => card.instanceId === command.cardInstanceId)!;
    const flurryPending = pending as PendingAttack & { flurryChoiceResolved?: boolean };
    if (selectedDefense.cardId === 'flurry-defensive-strikes' && !flurryPending.flurryChoiceResolved) {
      flurryPending.flurryChoiceResolved = true;
      if (!blessedMightCancelsDefenseCard(pending, selectedDefense.cardId, 'after-combat')) {
        const attacker = state.players[pending.attackerId];
        if (attacker.hand.some((card) => !cardDefinition(card).cannotBeDiscarded)) {
          state.flurry = { defenderId: defender.id, attackerId: attacker.id, resumePhase: 'defending', resumeDefenseCommand: command, remainingEnemyDiscards: 0 };
          state.phase = 'flurry-offer';
          state.log.unshift(`${defender.name} may lose 1 HP to force ${attacker.name} to discard 1 Card before combat.`);
          return ok(state);
        }
        state.log.unshift(`Flurry's optional effect was unavailable because ${attacker.name} had no eligible Card to discard.`);
      }
    }
  }
  ...
  if (defender.character === 'john-christ') pending.defenderWasInSpiritForm ??= defenderSpiritFormAtCombatStart;
  const shieldEquippedAtDefenseStart = defender.shieldEquipped;
  if (command.type === 'defend') {
    const selectedDefense = defender.hand.find((card) => card.instanceId === command.cardInstanceId);
    if (!selectedDefense || cardDefinition(selectedDefense).kind !== 'defend') return fail(state, 'That Defend card is not in the hand.');
    if (spiritFormBlocksCard(defender, cardDefinition(selectedDefense))) return fail(state, 'John Christ cannot use Cards containing “Bless” while in Spirit Form.');
    if (selectedDefense.cardId === 'yamato' && !(pending as PendingAttack & { yamatoMoveResolved?: boolean }).yamatoMoveResolved && !blessedMightCancelsDefenseCard(pending, selectedDefense.cardId)) {
      if (defender.hand.some((card) => card.cardId === 'panic')) {
        (pending as PendingAttack & { yamatoMoveResolved?: boolean }).yamatoMoveResolved = true;
        state.log.unshift(`Yamato's before-combat movement was skipped because Panic prevents ${defender.name} from moving.`);
      } else {
    ...
    if (selectedDefense.cardId === 'blink' && !(pending as BlinkPendingAttack).blinkMoveResolved && !blessedMightCancelsDefenseCard(pending, selectedDefense.cardId)) {
      ...
      if (defender.manaPoints === 0 && !defender.hand.some((card) => blinkHandCostEligible(card, selectedDefense.instanceId))) {
        blink.blinkMoveResolved = true;
        state.log.unshift(`Blink has no extra cost available, so ${defender.name} uses it as a 0 Value Defend Card without teleporting.`);
      } else {
        blink.blinkMoveResolved = true;
        blink.blinkDefenseCommand = command;
        if (defender.manaPoints > 0) {
          ...
          state.log.unshift(`Blink was revealed without Mana. ${defender.name} must discard another Card from Hand before choosing a teleport destination.`);
        }
        return ok(state);
      }
    }
  ...
  if (!pending.blessingFaithApplied) {
    ...
    if (faithPlayer) {
      ...
      const defendBase = defenseInstance ? cardBaseValue(defenseInstance) : 0;
      state.combatReveal = { attackCardId: pending.cardId, defendCardId, attackBase: cardDefinition({ instanceId: '', cardId: pending.cardId }).value, attackTotal: pending.attackValue, defendBase, defendTotal: defendBase + graveyardDefenseBonus(pending, defendCardId), expiresAt: Date.now() + 86_400_000, acknowledged: [], blessingFaith: { defenseCommand: command, playerId: faithPlayer.id } };
  ...
  if (pending.blessingMightApplied === undefined) {
    ...
    if (blessing && !pending.attackerWasInSpiritForm && !attacker.brainFreezeCombatBlocked) {
      ...
      if (command.type === 'defend') {
        ...
        defendCardId = instance.cardId; defendBase = cardBaseValue(instance);
      }
      state.combatReveal = { attackCardId: pending.cardId, defendCardId, attackBase: cardDefinition({ instanceId: '', cardId: pending.cardId }).value, attackTotal: pending.attackValue, defendBase, defendTotal: defendBase + graveyardDefenseBonus(pending, defendCardId), expiresAt: Date.now() + 86_400_000, acknowledged: [], blessingMight: { defenseCommand: command, playerId: attacker.id } };
  ...
  if (pending.blessingShieldApplied === undefined) {
    ...
    if (shieldPlayer) {
      ...
      if (command.type === 'defend') {
        ...
        defendBase = cardBaseValue(instance);
      }
      state.combatReveal = { attackCardId: pending.cardId, defendCardId, attackBase: cardDefinition({ instanceId: '', cardId: pending.cardId }).value, attackTotal: pending.attackValue, defendBase, defendTotal: defendBase + graveyardDefenseBonus(pending, defendCardId), expiresAt: Date.now() + 86_400_000, acknowledged: [], mythrilHelmet: { defenseCommand: command, playerId: shieldPlayer.id } };
  ...
  if (pending.mythrilHelmetApplied === undefined) {
    ...
    if (helmet) {
      ...
      state.combatReveal = { attackCardId: pending.cardId, defendCardId, attackBase: cardDefinition({ instanceId: '', cardId: pending.cardId }).value, attackTotal: pending.attackValue, defendBase, defendTotal: defendBase + graveyardDefenseBonus(pending, defendCardId), expiresAt: Date.now() + 86_400_000, acknowledged: [], mythrilHelmet: { defenseCommand: command, playerId: defender.id } };
      state.phase = 'choosing-mythril-helmet';
  ...
  if (pending.cardId === 'mana-barrage' && attackerForManaBarrage.manaMode !== 'consume' && pending.manaBarrageManaApplied === undefined) {
    if (attackerForManaBarrage.manaPoints > 0 && !attackerForManaBarrage.brainFreezeCombatBlocked) {
      ...
      state.combatReveal = { attackCardId: pending.cardId, defendCardId, attackBase: cardDefinition({ instanceId: '', cardId: pending.cardId }).value, attackTotal: pending.attackValue, defendBase, defendTotal: defendBase + graveyardDefenseBonus(pending, defendCardId), expiresAt: Date.now() + 86_400_000, acknowledged: [], manaBarrage: { defenseCommand: command, playerId: pending.attackerId } };
  ...
  if (!mockeryResolved) {
    ...
    if (eligible.length > 0) {
      ...
      state.log.unshift(`Vicious Mockery decision: ${eligible.map((id) => state.players[id].name).join(' and ')} may Remove their strongest available copy for its listed bonus to their combat Card.`);
      return ok(state);
    }
  }
  if (!exhaustResolved) {
    ...
    if (eligible.length > 0) {
      state.combatReveal = { attackCardId: pending.cardId, defendCardId: previewDefenseCard, attackBase: cardDefinition({ instanceId: '', cardId: pending.cardId }).value, attackTotal: pending.attackValue, defendBase: previewDefenseBase, defendTotal: Math.max(0, previewDefenseTotal + defenderMockery), expiresAt: Date.now() + 86_400_000, acknowledged: [], exhaust: { defenseCommand: command, eligible, decided: [], attached: [], defenderMockery } };
      state.phase = 'choosing-exhaust';
      state.log.unshift(`Exhaust decision: ${eligible.map((id) => state.players[id].name).join(' and ')} may attach one Exhaust for -2 Value.`);
      return ok(state);
    }
  }
  resolveSoulStrikeAfterDefenseChosen(state, pending, command);
  if (defender.hp <= 0) return ok(state);
  let defenseValue = 0;
  let defenseBaseValue = 0;
  let defenseCardId: CardTypeId | null = null;
  let carianReturnDefenseInstanceId: string | null = null;
  let defendModifiers: CombatModifier[] = [];
  let tacticianEmpowered = false;
  if (command.type === 'defend') {
    const instance = defender.hand.find((card) => card.instanceId === command.cardInstanceId);
    if (!instance || cardDefinition(instance).kind !== 'defend') return fail(state, 'That Defend card is not in the hand.');
    ...
    const defenseEffectsCancelled = blessedMightCancelsDefenseCard(pending, defenseCardId);
    defenseBaseValue = cardBaseValue(instance);
    ...
    const helmetDefenseBonus = pending.combatStackDefenderHelmet ? 2 : 0;
    const manaShieldBonus = defenseCard.id === 'mana-shield' && !defenseEffectsCancelled ? defender.manaPoints : 0;
    const lightsaberDefenseBonus = defender.character === 'shinobi' && defender.lightsaberBuff && !defender.traitBlocked ? 1 : 0;
    const equippedShieldBonus = defender.character === 'orkk' && shieldEquippedAtDefenseStart && defenseCard.id !== 'mana-baryer' ? 1 : 0;
    const manaBaryerTransformation = defenseCard.id === 'mana-baryer' && shieldEquippedAtDefenseStart && !defenseEffectsCancelled ? 5 - cardBaseValue(instance) : 0;
    ...
    const graveyardBonus = graveyardDefenseBonus(pending, defenseCard.id);
    ...
    defendModifiers = [
      ...
      helmetDefenseBonus && { value: helmetDefenseBonus, source: 'Helmet' },
      ...
      defenderAttachedExhaust && { value: -2, source: 'attached Exhaust' },
  ...
  if (pending.redirect && damage > 0) {
    const object = nextRedirectObject(state, defender);
    if (object) {
      damage -= 1;
  ...
  if (pending.attackValue > defenseValue && !blinkMissed) {
    ...
    if (potato?.carrierId === defender.id) {
      removeHotPotatoCard(defender);
      attackerBeforeCombatEffects.hand.push({ instanceId: `${attackerBeforeCombatEffects.id}-hot-potato-${++instanceSequence}`, cardId: 'hot-potato', revealedToOpponent: true });
      potato.carrierId = attackerBeforeCombatEffects.id;
      state.log.unshift(`${attackerBeforeCombatEffects.name} stole the Hot Potato from ${defender.name} with a successful Attack.`);
    }
  }
  ...
  const afterCombatDamageEventStart = ((state as GameState & { damageLog?: DamageLogEntry[] }).damageLog ?? []).length;
  ...
  if (blinkMissed) state.log.unshift(`Blink made the Attack miss ${defender.name}: all Damage and negative effects on the defender were prevented, while other combat effects continued.`);
  ...
  if (!attackEffectsCancelled && !attackCardDebuffsPrevented && pending.cardId === 'bone-chill') {
    stealWrecknaMovement(state, attackerAfterCombat, defender, pending.boneChillSteal ?? 1, 'Bone Chill');
  }
  ...
  if (pending.cardId === 'hello-there') {
    ...
    if (attackEffectsCancelled && additionalDamage > 0) {
      ...
      recordCombatDamageBlocked(state, defender, additionalDamage);
      state.log.unshift(`${defenseCardId ? cardDefinition({ instanceId: '', cardId: defenseCardId }).name : 'The Defence effect'} negated ${additionalDamage} additional damage from Hello There.`);
  ...
  const afterCombatAttackBlessingStart = state.blessingAnimations.length;
  if (!attackEffectsCancelled && pending.cardId === 'blessed-light') {
    if (!blessingShieldBlocksCombatStatus(state, defender, 'exhaust')) {
      if (addForcedStatusCard(state, defender, 'exhaust', 'deck', pending.attackerId, 'attack', false) && defender.deck.length > 1) defender.deck = shuffle(defender.deck);
    }
    const john = state.players[pending.attackerId];
    addBlessingCardToJohn(state, john, 'blessing-light', 'attack', !pending.attackerWasInSpiritForm);
  ...
  if (!attackEffectsCancelled && pending.cardId === 'blessed-might') {
    const john = state.players[pending.attackerId];
    addBlessingCardToJohn(state, john, 'blessing-might', 'attack', !pending.attackerWasInSpiritForm);
    state.log.unshift(`Blessed Might created Blessing: Might for ${john.name} after combat.`);
  }
  const afterCombatAttackEffectStart = state.spellProjectiles.length;
  if (!attackEffectsCancelled && !attackCardDebuffsPrevented && pending.cardId === 'cleanse' && defender.hp > 0) {
    if (!blessingShieldBlocksCombatStatus(state, defender, 'burning')) {
      const applied = addForcedStatusCard(state, defender, 'burning', 'hand', pending.attackerId, 'attack', true);
      if (applied) {
        const targetPosition = pending.defenderPosition ?? defender.position;
        state.spellProjectiles.push({ id: `${state.turn}-cleanse-immolate-${defender.id}-${++instanceSequence}`, casterId: pending.attackerId, targetId: defender.id, from: { ...targetPosition }, to: { ...targetPosition }, path: [{ ...targetPosition }, { ...targetPosition }], count: 1, damage: 0, style: 'cleanse-immolate' });
      }
      state.log.unshift(`Cleanse applied a Burning Status Card to ${defender.name}'s Hand after combat.`);
    }
  }
  if (!attackEffectsCancelled && !attackCardDebuffsPrevented && pending.cardId === 'excalibur') {
    if (!blessingShieldBlocksCombatStatus(state, defender, 'headache')) {
      addForcedStatusCard(state, defender, 'headache', 'hand', pending.attackerId, 'attack', true);
  ...
  if (!attackEffectsCancelled && pending.cardId === 'sting') {
    resolveStingAfterCombat(state, state.players[pending.attackerId], pending.cardInstanceId);
  }
  if (!attackEffectsCancelled && pending.cardId === 'judgement' && pending.attackValue > defenseValue) {
    const john = state.players[pending.attackerId];
    john.stoicShell = true;
    state.log.unshift(`Judgement won combat and granted Stoic Shell to ${john.name}.`);
  }
  if (!attackEffectsCancelled && !attackCardDebuffsPrevented && pending.cardId === 'frostmourne') {
    if (!blessingShieldBlocksCombatStatus(state, defender, 'exhaust')) {
      if (addForcedStatusCard(state, defender, 'exhaust', 'deck', pending.attackerId, 'attack', false)) {
        defender.knownTopCardId = 'exhaust';
        state.log.unshift(`Frostmourne added Exhaust on top of ${defender.name}'s Deck after combat.`);
      }
    }
  }
  if (!attackEffectsCancelled && !attackCardDebuffsPrevented && pending.cardId === 'enforce' && defender.hp > 0) {
    if (!blessingShieldBlocksCombatStatus(state, defender, 'panic')) addForcedStatusCard(state, defender, 'panic', 'hand', pending.attackerId, 'attack', true);
    if (!blessingShieldBlocksCombatStatus(state, defender, 'headache')) addForcedStatusCard(state, defender, 'headache', 'hand', pending.attackerId, 'attack', true);
  ...
  if (!attackEffectsCancelled && pending.cardId === 'echo-strike') {
    const replicas = spectreReplicas(state, pending.attackerId);
    const victims = echoStrikeVictims(state, pending.attackerId);
  ...
  if (!attackEffectsCancelled && !attackCardDebuffsPrevented && pending.cardId === 'displace' && defender.hp > 0) {
    ...
    if (blocked) {
      ...
      moveBoardObject(state, targetObject, destination);
  ...
  if (!attackEffectsCancelled && pending.cardId === 'repent') {
    const john = state.players[pending.attackerId];
    state.spellProjectiles.push({ id: `${state.turn}-repent-fire-${++instanceSequence}`, casterId: john.id, targetId: john.id, from: { ...john.position }, to: { ...john.position }, path: [], count: 1, damage: 0, style: 'repent-fire' });
    const impactTarget = pending.defenderBody === 'replica' ? state.objects.find((entry) => entry.id === pending.defenderReplicaId) : defender;
  ...
  if (!attackEffectsCancelled && pending.cardId === 'arcane-bolt') {
    ...
    const previousBonus = logan.arcaneBoltAttackBonus;
    logan.arcaneBoltAttackBonus = attackerBeforeCombatEffects.manaMode === 'consume' ? 2 : 1;
    showStatEffect(state, logan, Math.max(0, logan.arcaneBoltAttackBonus - previousBonus), 'ATT');
  ...
  if (!attackEffectsCancelled && !attackCardDebuffsPrevented && pending.cardId === 'teef-strike') {
    if (!blessingShieldBlocksCombatStatus(state, defender, 'exhaust')) {
      addForcedStatusCard(state, defender, 'exhaust', 'hand', pending.attackerId, 'attack', true);
  ...
  if (!attackEffectsCancelled && !attackCardDebuffsPrevented && pending.cardId === 'chip-cast') {
    ...
    for (let index = 0; index < headacheCount; index++) if (!blessingShieldBlocksCombatStatus(state, defender, 'headache')) addForcedStatusCard(state, defender, 'headache', 'discard', pending.attackerId, 'attack', true);
  ...
  if (!attackEffectsCancelled && pending.cardId === 'shield-bash' && !pending.shieldEquippedAtStart) {
    ...
    if (recall) {
      ...
      if (path.length > 0) {
        ...
        const crossedEnemyIds = new Set<string>();
        for (const [pathIndex, cell] of path.entries()) {
          const enemy = enemyBodyAt(state, orkk.id, cell);
          const enemyKey = enemy ? `${enemy.kind}:${enemy.id}` : '';
          if (!enemy || crossedEnemyIds.has(enemyKey)) continue;
          crossedEnemyIds.add(enemyKey);
          const damageAnimationStart = state.objectPushAnimations.length;
          combatDamageCharacterBody(state, enemy, 2, orkk.id, 'attack', true);
          for (const event of state.objectPushAnimations.slice(damageAnimationStart)) {
            if (!event.damage?.collision) continue;
            event.damage.triggerAnimationId = recallAnimationId;
            event.damage.triggerRouteProgress = (pathIndex + 1) / path.length;
  ...
  if (defenseCardId === 'arcane-shield' && shieldEquippedAtDefenseStart && !defenseEffectsCancelled) {
    ...
    for (const enemy of adjacentEnemies) combatDamageCharacterBody(state, enemy, 1, defender.id, 'defense');
    state.log.unshift(`Arcane Shield dealt 1 Damage to ${adjacentEnemies.map((enemy) => enemy.name).join(', ') || 'no adjacent enemies'}.`);
  }
  if (defenseCardId === 'yamato' && !defenseEffectsCancelled) {
    if (pending.attackValue > defenseValue) {
      const adjacentEnemies = enemyBodies(state, defender.id).filter((body) => distance(defender.position, body.position) === 1);
      for (const enemy of adjacentEnemies) combatDamageCharacterBody(state, enemy, 1, defender.id, 'defense');
      state.log.unshift(`Yamato lost combat and dealt 1 Damage to ${adjacentEnemies.map((enemy) => enemy.name).join(', ') || 'no adjacent enemies'}.`);
    }
    grantMerylinSummon(state, defender.id, 'Yamato');
  }
  if (defenseCardId === 'oracle' && !defenseEffectsCancelled) {
    const revealCount = defenseBaseValue;
    const attacker = state.players[pending.attackerId];
    const hiddenCards = attacker.hand.filter((card) => !card.revealedToOpponent && !card.revealedToPlayerIds?.includes(defender.id));
    const revealedCards = hiddenCards.slice(0, revealCount);
    for (const card of revealedCards) card.revealedToOpponent = true;
    state.log.unshift(`Oracle revealed ${revealedCards.length} of ${attacker.name}'s Cards after combat${revealedCards.length < revealCount ? ` (${revealCount} requested, but only ${revealedCards.length} hidden Card${revealedCards.length === 1 ? ' was' : 's were'} available)` : ''}.`);
  }
  if (defenseCardId === 'dispersion' && !defenseEffectsCancelled) {
    const reflectedDamage = Math.min(4, damage);
    const adjacentEnemies = enemyBodies(state, defender.id).filter((body) => distance(body.position, defenderCombatPosition) === 1);
    for (const enemy of adjacentEnemies) combatDamageCharacterBody(state, enemy, reflectedDamage, defender.id, 'defense');
  ...
  if (defenseCardId === 'countaspell' && !defenseEffectsCancelled) {
    ...
    for (let index = 0; index < headacheCount; index++) if (!blessingShieldBlocksCombatStatus(state, attacker, 'headache')) addForcedStatusCard(state, attacker, 'headache', 'discard', defender.id, 'defense', true);
  ...
  if (defenseCardId === 'mana-baryer' && !shieldEquippedAtDefenseStart && !defenseEffectsCancelled) {
    ...
    if (recall) {
      ...
      if (path.length > 0) {
        ...
        const crossedEnemyIds = new Set<string>();
        for (const [pathIndex, cell] of path.entries()) {
          const enemy = enemyBodyAt(state, defender.id, cell);
          const enemyKey = enemy ? `${enemy.kind}:${enemy.id}` : '';
          if (!enemy || crossedEnemyIds.has(enemyKey)) continue;
          crossedEnemyIds.add(enemyKey);
          const damageAnimationStart = state.objectPushAnimations.length;
          combatDamageCharacterBody(state, enemy, 2, defender.id, 'defense', true);
          for (const event of state.objectPushAnimations.slice(damageAnimationStart)) {
            if (!event.damage?.collision) continue;
            event.damage.triggerAnimationId = recallAnimationId;
            event.damage.triggerRouteProgress = (pathIndex + 1) / path.length;
  ...
  if (defenseCardId === 'arcane-barrier' && !defenseEffectsCancelled) {
    const attacker = state.players[pending.attackerId];
    const attackerReplica = attackingSpectreReplica(state, pending);
    const attackerPosition = attackerReplica?.position ?? attacker.position;
    const defenderPosition = pending.defenderPosition ?? defender.position;
    if (distance(defenderPosition, attackerPosition) !== 1) {
      state.log.unshift(`Arcane Barrier could not affect ${attacker.name} because the attacker was not adjacent to ${defender.name}.`);
    } else {
      const dx = Math.sign(attackerPosition.x - defenderPosition.x);
      const dy = Math.sign(attackerPosition.y - defenderPosition.y);
      const destination = { x: attackerPosition.x + dx, y: attackerPosition.y + dy };
      ...
      const barrierAnimationId = `${state.turn}-arcane-barrier-pulse-${++instanceSequence}`;
      state.objectPushAnimations.push({
        id: barrierAnimationId, objectId: '',
        from: { ...attackerPosition }, to: blocked ? { ...attackerPosition } : { ...destination },
        dx, dy, collided: blocked, arcaneBarrier: { defenderPosition: { ...defenderPosition }, targetPlayerId: attackerReplica ? undefined : attacker.id, targetObjectId: attackerReplica?.id, waitForAttackEffectIds: [...state.spellProjectiles.slice(afterCombatAttackEffectStart).map((effect) => effect.id), ...state.blessingAnimations.slice(afterCombatAttackBlessingStart).filter((effect) => effect.playerId === attacker.id && effect.source === 'attack').map((effect) => effect.id)] },
      });
      if (blocked) {
        const damageEventStart = state.objectPushAnimations.length;
        dealCombatCardEffectDamage(state, attacker, 1, defender.id, 'defense');
        for (const event of state.objectPushAnimations.slice(damageEventStart)) if (event.damage) event.afterBarrierAnimationId = barrierAnimationId;
        state.log.unshift(`Arcane Barrier could not push ${attacker.name} and dealt 1 Damage instead.`);
      } else if (attackerReplica) {
        const origin = { ...attackerReplica.position };
        moveBoardObject(state, attackerReplica, destination);
        pending.attackerPosition = { ...destination };
        state.objectPushAnimations.push({ id: `${state.turn}-arcane-barrier-${attackerReplica.id}-${++instanceSequence}`, objectId: attackerReplica.id, from: origin, to: destination, dx, dy, collided: false, path: [destination] });
        applyElevationDropDamage(state, { kind: 'object', id: attackerReplica.id, position: origin }, origin, destination, defender.id, 'other');
        state.log.unshift(`Arcane Barrier pushed ${attacker.name}'s attacking replica 1 Square away from ${defender.name}.`);
      } else {
  ...
  if (defenseCardId === 'counterspell' && !defenseEffectsCancelled) {
    ...
    if (!blessingShieldBlocksCombatStatus(state, counterTarget, 'headache')) {
      addForcedStatusCard(state, counterTarget, 'headache', 'deck', defender.id, 'defense', false);
  ...
  if (defenseCardId === 'blink' && !defenseEffectsCancelled && (pending as BlinkPendingAttack).blinkDestination) {
    const blink = pending as BlinkPendingAttack;
    state.log.unshift(`${defender.name} paid Blink's extra cost by ${blink.blinkManaSpent ? `removing ${blink.blinkManaSpent} Mana` : 'discarding a Card from Hand'} and will teleport after the combat window closes.`);
  }
  ...
  if (attacker.hp === 0 || defender.hp === 0) {
  ...
  else if (!attackEffectsCancelled && !attackCardDebuffsPrevented && pending.cardId === 'force-disarm') {
    ...
    if (attackCards.length > 0) {
      ...
      if (!blessingShieldBlocksCombatStatus(state, defender, 'exhaust')) addForcedStatusCard(state, defender, 'exhaust', 'hand', pending.attackerId, 'attack', true);
      state.phase = 'active';
      state.log.unshift(`${defender.name} had no Attack Card; Force Disarm revealed their Hand and added an Exhaust Card to it.`);
    }
  }
  else if (!attackEffectsCancelled && !attackCardDebuffsPrevented && pending.cardId === 'teef-strike') {
    ...
    state.phase = 'dance-through';
    state.danceThrough = { playerId: pending.attackerId, stepsRemaining: 3, enemyUnderfoot: null, objectUnderfoot: null, damagePrevented: false, pinnedEnemyIds: [] } as typeof state.danceThrough & { objectUnderfoot: string | null; pinnedEnemyIds: PlayerId[] };
  ...
  if (!attackEffectsCancelled && !attackCardDebuffsPrevented && pending.cardId === 'grimoire-cleanse' && damage > 0 && state.phase !== 'finished') {
  ...
  if (!attackEffectsCancelled && !attackCardDebuffsPrevented && pending.cardId === 'enfeeble' && state.phase !== 'finished') {
    ...
    if (attackCards.length > 0) {
      ...
      addForcedStatusCard(state, defender, 'exhaust', 'hand', attacker.id, 'attack', true);
      state.log.unshift(`Enfeeble found no Attack Card to discard and added Exhaust to ${defender.name}'s Hand.`);
    }
  }
  if (!attackEffectsCancelled && !attackCardDebuffsPrevented && pending.cardId === 'finger-of-death' && state.phase !== 'finished') {
    addForcedStatusCard(state, defender, 'headache', 'deck', attacker.id, 'attack', true);
    state.log.unshift(`Finger of Death added Headache on top of ${defender.name}'s Deck.`);
  }
  ...
  if (defenseCardId === 'feed-the-spirit' && !defenseEffectsCancelled && state.phase !== 'finished') {
    ...
    if (blessings.length > 0) {
      ...
      state.phase = 'mana-blast-offer';
      postCombatChoicePending = true;
      state.log.unshift(`${defender.name} may Remove a Blessing Card to restore ${pending.feedSpiritCombatDamage ?? 0} Hit Point${(pending.feedSpiritCombatDamage ?? 0) === 1 ? '' : 's'} lost from combat Damage with Feed the Spirit.`);
    }
  }
  ...
  if (defenseCardId === 'split' && !defenseEffectsCancelled && state.phase !== 'finished') {
    (state as SpectreTargetingState).pendingSplit = { defenderId: defender.id, origin: { ...defender.position } };
    postCombatChoicePending = true;
  }
  ...
  if (defenseCardId === 'anguish' && !defenseEffectsCancelled && state.phase !== 'finished') {
    (state as SpectreTargetingState).pendingAnguish = { defenderId: defender.id, attackerId: attacker.id, sufferedDamage: defenderSufferedDamage };
    postCombatChoicePending = true;
  }
  ...
  if (!attackEffectsCancelled && pending.cardId === 'frostmourne' && state.phase !== 'finished') {
    const attacker = state.players[pending.attackerId];
    if (beginFrostmourneChoice(state, attacker, pending.cardInstanceId, state.phase)) postCombatChoicePending = true;
  }
  if (carianReturnDefenseInstanceId && returnDiscardedCardToHand(defender, carianReturnDefenseInstanceId)) state.log.unshift(`Carian Stance returned ${cardDefinition({ instanceId: '', cardId: defenseCardId! }).name} to ${defender.name}'s Hand after combat.`);
  if (!postCombatChoicePending) state.pendingAttack = null;
  if ((pending as PendingAttack & { flurryDiscardAfterCombat?: boolean }).flurryDiscardAfterCombat && state.phase !== 'finished') {
    if (attacker.hand.some((card) => !cardDefinition(card).cannotBeDiscarded)) {
      state.flurry = { defenderId: defender.id, attackerId: attacker.id, resumePhase: state.phase, remainingEnemyDiscards: 1 };
      state.phase = 'choosing-flurry-enemy-discard';
      state.log.unshift(`${attacker.name} must discard 1 Card after the combat window closes.`);
    } else state.log.unshift(`${attacker.name} had no Card eligible for Flurry's forced discard after combat.`);
  }
  state.combatReveal = null;
  ...
  if (blink.blinkOrigin && blink.blinkDestination) {
    ...
    (stateBeforeAfterCombatEffects.combatReveal as BlinkCombatReveal).blinkTeleport = {
      defenderId: defender.id, from: blink.blinkOrigin, to: blink.blinkDestination, missed: Boolean(blink.blinkMissed),
    };
  }
...
function resumeFlurryCombat(state: GameState, flurry: NonNullable<GameState['flurry']>): CommandResult {
  state.phase = flurry.resumePhase;
  state.flurry = null;
  return flurry.resumeDefenseCommand ? resolveDefense(state, flurry.resumeDefenseCommand) : ok(state);
}
...
function resolveFlurryDecline(state: GameState, playerId: PlayerId): CommandResult {
  ...
  return resumeFlurryCombat(state, flurry);
}

function resolveFlurryPay(state: GameState, playerId: PlayerId, _cardInstanceId: string): CommandResult {
  const flurry = state.flurry;
  ...
  const defender = state.players[playerId];
  {
    ...
    if (defender.hp === 0) {
      if (flurry.resumeDefenseCommand?.type === 'defend') discardFromHand(defender, flurry.resumeDefenseCommand.cardInstanceId);
      registerCharacterDefeat(state, defender, attacker.id);
      state.pendingAttack = null;
      state.flurry = null;
      state.phase = state.winner ? 'finished' : 'active';
      return ok(state);
    }
    if (state.pendingAttack) (state.pendingAttack as PendingAttack & { flurryDiscardAfterCombat?: boolean }).flurryDiscardAfterCombat = true;
    return resumeFlurryCombat(state, flurry);
  }
}
...
function resolveForceDisarmDiscard(state: GameState, playerId: PlayerId, cardInstanceId: string): CommandResult {
  ...
  const automaticBanner = card?.cardId === 'banner' || card?.cardId === 'banner-draw';
  if (!card || cardDefinition(card).cannotBeDiscarded || (!automaticBanner && !mindBlast && cardDefinition(card).kind !== requiredKind)) return fail(state, mindBlast ? 'Mind Blast requires a discardable Card.' : `${source} requires a ${requiredKind === 'attack' ? 'Attack' : 'Defend'} Card to be discarded.`);
...
function snapshotPerkTargeting(player: PlayerState): PerkTargetingUndo {
  return structuredClone({ deck: player.deck, hand: player.hand, discard: player.discard, spellEcho: player.spellEcho, actionsRemaining: player.actionsRemaining, perkUsed: player.perkUsed, manaPoints: player.manaPoints, dakkothRangeBonus: player.dakkothRangeBonus, spellsingerExtraPerkUses: player.spellsingerExtraPerkUses });
}
...
function resolveDakkothTombSquare(state: GameState, playerId: PlayerId, to: Cell): CommandResult {
  ...
  if (!wrecknaPerkTargetInRange(state, caster, to)) return fail(state, `Dakkoth must create its Tomb within Range ${effectiveAttackRange(state, caster)} and line of sight.`);
...
function finishSap(state: GameState, pending: NonNullable<WrecknaChoiceState['sap']>, target: PlayerState): CommandResult {
  if (pending.level >= 3) {
    ...
    if (echoIndex == null) state.log.unshift(`Sap level 3 found no Perk in ${target.name}'s Spell Echo.`);
      ...
      if (!discarded.oneTimeCopy) {
        ...
        discarded.revealedToPlayerIds = [];
        target.discard.push(prepareCardForDiscard(discarded));
      }
  ...
  (state as WrecknaChoiceState).sap = null;
  state.phase = 'active';
  return ok(state);
}
...
function resolveSapTarget(state: GameState, playerId: PlayerId, targetId: string, targetKind: 'player' | 'replica' = 'player'): CommandResult {
  ...
  const body = characterBody(state, targetKind, targetId);
  const target = body ? state.players[body.ownerId] : undefined;
  if (!body || !target || body.ownerId === playerId || target.hp <= 0) return fail(state, 'Sap requires a living enemy target.');
  if (target.spectreShadowCloakActive) return fail(state, 'Shadow Cloak makes that character ineligible as a Perk target.');
  if (!wrecknaPerkTargetInRange(state, caster, body.position, pending.range)) return fail(state, `Sap requires an enemy within Range ${pending.range} and line of sight.`);
  pending.targetId = target.id;
  pending.targetKind = targetKind;
  const defendCards = target.hand.filter((card) => cardDefinition(card).kind === 'defend');
  if (defendCards.length === 0) {
    const message = `${target.name} has no Defend Card to reveal for Sap.`;
    (state as PrivateNoticeState).privateNotice = { id: `sap-no-defend-${state.turn}-${++instanceSequence}`, playerId, message, createdAt: Date.now() };
...
function resolveSapDefendReveal(state: GameState, playerId: PlayerId, cardInstanceId: string): CommandResult {
  const pending = (state as WrecknaChoiceState).sap;
  if (state.phase !== 'choosing-sap-defend' || !pending?.targetId || pending.targetId !== playerId) return fail(state, 'Sap is not waiting for this player to reveal a Defend Card.');
...
function resolveDecayTarget(state: GameState, playerId: PlayerId, targetId: string, targetKind: 'player' | 'replica' = 'player'): CommandResult {
  ...
  if ((state.phase as string) !== 'choosing-decay-target' || !pending || pending.casterId !== playerId) return fail(state, 'Curse is not waiting for a target.');
  const caster = state.players[playerId];
  const body = characterBody(state, targetKind, targetId);
  const target = body ? state.players[body.ownerId] : undefined;
  ...
  showStatEffect(state, target, -1, 'MOV');
  ...
  if (pending.level >= 3) {
    ...
    state.log.unshift(`Curse level 3 blocked ${target.name}'s Trait until the end of their turn.`);
  }
  extended.decay = null;
  state.phase = 'active';
  return ok(state);
...
function resolveDecayDiscard(state: GameState, playerId: PlayerId, cardInstanceId: string): CommandResult {
  const extended = state as WrecknaChoiceState;
  const pending = extended.decay;
  if ((state.phase as string) !== 'choosing-decay-discard' || !pending || pending.targetId !== playerId || pending.remaining <= 0) return fail(state, 'Curse is not waiting for this discard.');
  const target = state.players[playerId];
  ...
  pending.remaining -= 1;
  state.log.unshift(`${target.name} discarded ${cardDefinition(card).name} because of Curse.`);
...
function resolveFireballTarget(state: GameState, playerId: PlayerId, targetId: string, targetKind: 'player' | 'replica' = 'player'): CommandResult {
  ...
  if (source === 'firebolt' && target.spectreShadowCloakActive) return fail(state, 'Shadow Cloak makes that character ineligible as a Perk target.');
  const range = 3;
...
const SWEET_POTATO_NEGATIVE_STATUSES: CardTypeId[] = ['pinned', 'headache', 'exhaust', 'burning', 'panic'];
export function sweetPotatoStatusCount(player: PlayerState): number {
  return player.deck.filter((card) => SWEET_POTATO_NEGATIVE_STATUSES.includes(card.cardId)).length;
}
function resolveSweetPotatoChoice(state: GameState, playerId: PlayerId, choice: 'heal' | 'cleanse' | 'cancel'): CommandResult {
  const extended = state as GameState & { sweetPotato?: { casterId: PlayerId; undo: PerkTargetingUndo } | null };
  if ((state.phase as string) !== 'choosing-sweet-potato' || extended.sweetPotato?.casterId !== playerId) return fail(state, 'Sweet Potato is not waiting for this choice.');
  const player = state.players[playerId];
  if (choice === 'cancel') {
    const undo = extended.sweetPotato.undo;
    player.deck = undo.deck; player.hand = undo.hand; player.discard = undo.discard; player.spellEcho = undo.spellEcho;
    player.actionsRemaining = undo.actionsRemaining; player.perkUsed = undo.perkUsed; player.spellsingerExtraPerkUses = undo.spellsingerExtraPerkUses ?? 0; player.manaPoints = undo.manaPoints; player.dakkothRangeBonus = undo.dakkothRangeBonus ?? 0;
    state.log.unshift(`${player.name} cancelled Sweet Potato and returned it to their Hand.`);
  } else if (choice === 'heal') {
    const healed = healPlayer(state, player, 2);
    state.log.unshift(`Sweet Potato restored ${healed} Hit Point${healed === 1 ? '' : 's'} to ${player.name}.`);
  } else {
    const removed = sweetPotatoStatusCount(player);
    player.deck = player.deck.filter((card) => !SWEET_POTATO_NEGATIVE_STATUSES.includes(card.cardId));
    state.log.unshift(`Sweet Potato Removed ${removed} negative Status Card${removed === 1 ? '' : 's'} from ${player.name}'s Deck.`);
  }
  extended.sweetPotato = null;
  state.phase = 'active';
  return ok(state);
}
...
function finishMindTricksSelection(state: GameState, playerId: PlayerId): CommandResult {
  ...
  if (mind.level >= 2) {
    if (addForcedStatusCard(state, enemy, 'headache', 'deck', playerId, 'perk', false)) enemy.deck = shuffle(enemy.deck);
    enemy.knownTopCardId = null;
...
function resolveBlinkDiscard(state: GameState, playerId: PlayerId, cardInstanceId: string): CommandResult {
  const pending = state.pendingAttack as BlinkPendingAttack | null;
  if (state.phase !== 'choosing-blink-discard' || pending?.defenderId !== playerId || !pending.blinkDefenseCommand) return fail(state, 'Blink is not waiting for this discard.');
  const player = state.players[playerId];
  const card = player.hand.find((entry) => entry.instanceId === cardInstanceId);
  if (!card) return fail(state, 'That Card is not in the Hand.');
  if (!blinkHandCostEligible(card, pending.blinkDefenseCommand.cardInstanceId)) return fail(state, 'Blink requires another discardable Card from Hand.');
  const name = cardDefinition(card).name;
  discardFromHand(player, cardInstanceId);
  state.phase = 'choosing-blink-teleport';
  state.log.unshift(`Blink found no Mana; ${player.name} discarded ${name} as an extra cost and must choose a visible empty Square.`);
  return ok(state);
}
...
export function mindBlastCanTarget(state: GameState, caster: PlayerState, target: PlayerState): boolean {
  return target.id !== caster.id && target.hp > 0 && !target.spectreShadowCloakActive && distance(caster.position, target.position) <= effectiveAttackRange(state, caster) && hasLineOfSight(state, caster.position, target.position);
}
...
function finishMindBlast(state: GameState, caster: PlayerState, target: PlayerState, level: number) {
  ...
  if (level >= 3 && target.hp > 0) {
    let added = 0;
    for (let index = 0; index < 2; index += 1) added += Number(addForcedStatusCard(state, target, 'headache', 'deck', caster.id, 'perk', true));
    if (added > 0) target.knownTopCardId = 'headache';
    state.log.unshift(`Mind Blast added 2 Headache Cards on top of ${target.name}'s Deck.`);
  }
  if (state.phase !== 'finished') state.phase = 'active';
}

function resolveArcaneMissleTarget(state: GameState, playerId: PlayerId, targetId: string, targetKind: 'player' | 'replica' = 'player'): CommandResult {
  ...
  const body = characterBody(state, targetKind, targetId);
  const target = body ? state.players[body.ownerId] : undefined;
  if (!body || !target || body.ownerId === playerId) return fail(state, 'Arcane Missile must target an enemy.');
  if (target.spectreShadowCloakActive) return fail(state, 'Shadow Cloak makes that character ineligible as a Perk target.');
  const positionalTarget = { ...target, position: body.position };
  const mindBlast = (state as GameState & { mindBlast?: { casterId: PlayerId; level: number } | null }).mindBlast;
  if (mindBlast) {
    if (!mindBlastCanTarget(state, caster, positionalTarget)) return fail(state, `Mind Blast requires an enemy within Range ${effectiveAttackRange(state, caster)} and line of sight.`);
    state.spellProjectiles.push({ id: `${state.turn}-mind-blast-${++instanceSequence}`, casterId: playerId, targetId: body.id, from: { ...caster.position }, to: { ...body.position }, path: [{ ...body.position }], count: 1, damage: 0, style: 'mind-blast' });
    state.arcaneMissle = null;
    const eligible = target.hand.some((card) => !cardDefinition(card).cannotBeDiscarded);
    if (eligible) {
      state.forceDisarm = { targetId: target.id, source: 'force-disarm', mindBlastLevel: mindBlast.level, mindBlastCasterId: playerId } as typeof state.forceDisarm & { mindBlastLevel: number; mindBlastCasterId: PlayerId };
      state.phase = 'choosing-force-disarm-discard';
  ...
  const path = arcaneMisslePath(state, caster, positionalTarget, missile.level);
  ...
  const dealt = damageCharacterBody(state, body, missile.damage, false, playerId, 'perk');
  state.spellProjectiles.push({ id: `${state.turn}-arcane-${++instanceSequence}`, casterId: playerId, targetId: body.id, from: { ...caster.position }, to: { ...body.position }, path, count: missile.level, damage: dealt });
  state.arcaneMissle = null;
  state.phase = 'active';
  gainManaFromResolvedSpell(state, caster);
  state.log.unshift(`${caster.name} cast Arcane Missile level ${missile.level} at ${body.name} for ${dealt} Damage${missile.damage === 3 ? ' with Consume' : ''}.`);
  return ok(state);
}
...
function preferredChainBounce(state: GameState, casterId: PlayerId, current: ChainTarget, range: number, remaining: number): ChainTarget | undefined {
  ...
  const enemies = [...new Map(Object.values(state.players).filter((player) => player.id !== casterId && !player.spectreShadowCloakActive).map((player) => [player.id, player])).values()];
  let routes: Route[] = [{ current, objects: state.objects, hp: new Map(Object.values(state.players).map((player) => [player.id, player.hp])), hits: 0, destroyed: 0 }];
  const compare = (a: Route, b: Route) => (b.hits + b.destroyed) - (a.hits + a.destroyed)
    || Math.min(b.hits, b.destroyed) - Math.min(a.hits, a.destroyed)
  ...
  for (let depth = 0; depth < remaining; depth++) {
    ...
    for (const route of routes) {
      ...
      for (const candidate of candidates) {
        if (candidate.kind === route.current.kind && candidate.id === route.current.id) continue;
        if (distance(route.current.position, candidate.position) > range || !hasLineOfSight(projected, route.current.position, candidate.position)) continue;
        const hp = new Map(route.hp);
        const object = candidate.kind === 'object' ? route.objects.find((entry) => entry.id === candidate.id) : undefined;
        const damageTarget = candidate.kind === 'player' ? candidate.id as PlayerId : object?.kind === 'spectre-replica' ? object.ownerId : undefined;
        if (damageTarget) hp.set(damageTarget, hp.get(damageTarget)! - 1);
        const destroysObject = candidate.kind === 'object' && !damageTarget;
        expanded.push({ current: candidate, first: route.first ?? candidate, hp,
          objects: destroysObject ? route.objects.filter((object) => object.id !== candidate.id) : route.objects,
          hits: route.hits + Number(Boolean(damageTarget) && damageTarget !== casterId), destroyed: route.destroyed + Number(destroysObject) });
      }
    }
    if (!expanded.length) break;
    expanded.sort(compare);
    routes = expanded.slice(0, 128);
    if (compare(routes[0], best) < 0) best = routes[0];
  }
  return best.first;
}
...
function resolveChainLightningTarget(state: GameState, playerId: PlayerId, targetId: string, targetKind: 'player' | 'replica' = 'player'): CommandResult {
  ...
  const strike = (from: Cell, target: ChainTarget, hop: number) => {
    ...
    if (target.kind === 'player') dealDamage(state, state.players[target.id as PlayerId], 1, false, playerId, 'perk');
      ...
      if (targetReplica) dealDamage(state, state.players[targetReplica.ownerId!], 1, false, playerId, 'perk');
      else destroyObject(state, target.id, playerId, 'Chain Lightning');
    }
    state.spellProjectiles.push({ id: `${state.turn}-chain-${++instanceSequence}`, casterId: playerId, targetId: target.id, from: { ...from }, to: { ...target.position }, path: [{ ...from }, { ...target.position }], count: 1, damage: target.kind === 'player' || targetReplica ? 1 : 0, style: 'lightning' });
    state.log.unshift(`Chain Lightning strike ${hop + 1} hit ${target.kind === 'player' ? state.players[target.id as PlayerId].name : targetObject?.name ?? 'an Object'}.`);
  };

  strike(caster.position, current, 0);
  for (let bounce = 0; bounce < chain.bounces; bounce++) {
    const next = preferredChainBounce(state, caster.id, current, chain.bounceRange, chain.bounces - bounce);
    if (!next) { state.log.unshift(`Chain Lightning stopped after ${bounce} completed bounce${bounce === 1 ? '' : 's'}: no target was in range and line of sight.`); break; }
...
function selectForcePullTarget(state: GameState, playerId: PlayerId, targetKind: 'player' | 'object', targetId: string): CommandResult {
  ...
  if (isShadowCloakedPerkTarget(state, targetKind, targetId)) return fail(state, 'Shadow Cloak makes that character ineligible as a Perk target.');
  ...
  for (const next of path.slice(0, steps)) { applyElevationDropDamage(state, target, previous, next, playerId, 'perk'); previous = next; }
  if (target.kind === 'player') {
    ...
    moveBoardObject(state, state.objects.find((object) => object.id === target.id)!, destination);
    recordObjectEffect(state, target.id, playerId, 'Force Pull');
  }
  const pulledReplica = target.kind === 'object' ? state.objects.find((object) => object.id === target.id && object.kind === 'spectre-replica' && object.ownerId !== playerId) : null;
  const statusTarget = target.kind === 'player' ? state.players[target.id as PlayerId] : pulledReplica?.ownerId ? state.players[pulledReplica.ownerId] : null;
  if (pull.level >= 3 && statusTarget) applyPinned(statusTarget, 1, state, playerId);
  state.log.unshift(`Force Pull moved ${entityName(state, target)} ${steps} square${steps === 1 ? '' : 's'} toward ${caster.name}${pull.level >= 3 && statusTarget ? ` and applied Pinned to ${statusTarget.name}` : ''}.`);
...
export function armDaWizPath(state: GameState, shield: BoardObject, orkkCell: Cell, range: number): Cell[] {
  ...
  const enemyAt = (cell: Cell) => Boolean(shield.ownerId && enemyBodyAt(state, shield.ownerId, cell));
  ...
  for (let step = 1; step <= range && frontier.length > 0; step++) {
    ...
    for (const current of frontier) {
      ...
      for (const next of neighbors) {
        ...
        const blockedByObject = state.objects.some((entry) => entry.id !== shield.id
          && !(entry.kind === 'spectre-replica' && entry.ownerId !== shield.ownerId)
          && entry.position.x === next.x && entry.position.y === next.y);
...
function pullEnemiesAlongShieldRecall(state: GameState, shield: BoardObject, orkkId: PlayerId, path: Cell[], sourceName: string, triggerAnimationId?: string): void {
  ...
  const passes: { body: CharacterBody; pathIndex: number }[] = [];
  for (const [pathIndex, cell] of path.entries()) {
    const enemy = enemyBodyAt(state, orkkId, cell);
    if (!enemy || passes.some((entry) => entry.body.kind === enemy.kind && entry.body.id === enemy.id)) continue;
    passes.push({ body: enemy, pathIndex });
  }
  for (const pass of passes.sort((a, b) => b.pathIndex - a.pathIndex)) {
    const enemy = pass.body;
    const pushTarget = pushEntityForBody(enemy);
    const current = getPushEntity(state, pushTarget.kind, pushTarget.id);
    if (!current) continue;
    const destination = {
      x: current.position.x + Math.sign(orkk.position.x - current.position.x),
      y: current.position.y + Math.sign(orkk.position.y - current.position.y),
    };
    const blocked = (destination.x === orkk.position.x && destination.y === orkk.position.y)
      || Object.values(state.players).some((entry) => !(current.kind === 'player' && entry.id === current.id) && entry.position.x === destination.x && entry.position.y === destination.y)
      || state.objects.some((entry) => entry.id !== shield.id && !(current.kind === 'object' && entry.id === current.id) && entry.position.x === destination.x && entry.position.y === destination.y);
    if (blocked) {
    ...
    if (current.kind === 'player') {
      const moved = state.players[current.id as PlayerId];
      recordQuestMovement(state, moved.id, 1, false, destination);
      moved.position = { ...destination };
      markCharacterMoved(moved, 'enemy-ability');
      if (triggerAnimationId) moved.visualMovement = { from, path: [{ ...destination }], triggerAnimationId, triggerRouteProgress: (pass.pathIndex + 1) / path.length };
    } else {
      moveBoardObject(state, state.objects.find((object) => object.id === current.id)!, destination);
      state.objectPushAnimations.push({ id: `${state.turn}-${sourceName}-pull-${current.id}-${++instanceSequence}`, objectId: current.id, from, to: { ...destination }, dx: Math.sign(destination.x - from.x), dy: Math.sign(destination.y - from.y), collided: false, path: [{ ...destination }] });
    }
    applyElevationDropDamage(state, current, from, destination, orkkId, 'other');
...
function resolveArmDaWizTarget(state: GameState, playerId: PlayerId, objectId: string): CommandResult {
  ...
  const shieldPasses: { body: CharacterBody; pathIndex: number }[] = [];
  for (const [pathIndex, cell] of path.entries()) {
    const enemy = enemyBodyAt(state, playerId, cell);
    if (!enemy || shieldPasses.some((entry) => entry.body.kind === enemy.kind && entry.body.id === enemy.id)) continue;
    shieldPasses.push({ body: enemy, pathIndex });
    if (arm.level >= 2) {
      const damage = 1 + meleeHighGroundDamageBonus(state, orkk, enemy.position, shield.position);
      const damageAnimationStart = state.objectPushAnimations.length;
      damageCharacterBody(state, enemy, damage, true, playerId, 'perk');
      for (const event of state.objectPushAnimations.slice(damageAnimationStart)) {
        if (!event.damage?.collision) continue;
        event.damage.triggerAnimationId = recallAnimationId;
        event.damage.triggerRouteProgress = (pathIndex + 1) / path.length;
...
function resolveArkaneArowTarget(state: GameState, playerId: PlayerId, target: Cell): CommandResult {
  ...
  const enemy = enemyBodyAt(state, playerId, collisionCell);
  const obstacle = state.objects.find((entry) => entry.position.x === collisionCell.x && entry.position.y === collisionCell.y && entry.id !== (enemy?.kind === 'replica' ? enemy.id : ''));
  ...
  if (enemy) {
    ...
    damageCharacterBody(state, enemy, collisionDamage, true, playerId, 'perk');
    ...
    if (throwState.level >= 3) {
      ...
      const pushTarget = pushEntityForBody(enemy);
      const blocked = pushEntity(state, pushTarget, dx, dy, 1, throwState.level, playerId, false, 'perk');
      if (!blocked) {
        const destination = getPushEntity(state, pushTarget.kind, pushTarget.id)!.position;
        if (pushTarget.kind === 'player') state.players[enemy.ownerId].visualMovement = { from: { ...enemy.position }, path: [{ ...destination }], triggerAnimationId: shieldAnimationId };
        pushed = true;
        state.log.unshift(`ARKANE AROW pushed ${enemy.name} to ${cellLabel(destination)}.`);
      } else {
        damageCharacterBody(state, enemy, 1, true, playerId, 'perk');
...
function moveShizzle(state: GameState, player: PlayerState, to: Cell): CommandResult {
  ...
  const shizzleBodies = shizzle as typeof shizzle & { bodyUnderfoot?: CharacterBody | null };
  const passedEnemy = shizzleBodies.bodyUnderfoot ?? (shizzle.enemyUnderfoot ? characterBody(state, 'player', shizzle.enemyUnderfoot) : null);
...
function pushEntity(state: GameState, entity: PushEntity, dx: number, dy: number, movement: number, level: number, casterId: PlayerId, dealCollisionDamage = true, sourceKind: 'attack' | 'perk' | 'other' = 'other', dealElevationDamage = true, collisionDamageBonus = 0, destroyObjectOnPlayerCollision = false): boolean {
  ...
  const finishObjectAnimation = (collided: boolean, forceDestroy = false) => {
    ...
    if (current) {
      const destroyedByRepeatedEffect = recordObjectEffect(state, entity.id, casterId, 'push');
      const destroyedByCollision = forceDestroy && destroyObject(state, entity.id, casterId, 'Force Throw collision with a Player');
      const destroyed = destroyedByRepeatedEffect || destroyedByCollision;
  ...
  while (remaining > 0) {
    ...
    const movementOrigin = { ...current.position };
    ...
    else moveBoardObject(state, state.objects.find((object) => object.id === current.id)!, next);
    travelled.push({ ...next });
    if (slidTo) travelled.push({ ...slidTo });
    if (dealElevationDamage) applyElevationDropDamage(state, current, movementOrigin, next, casterId, sourceKind);
    remaining -= 1;
  }
...
function resolveSpectreReplicaSquare(state: GameState, playerId: PlayerId, to: Cell): CommandResult {
  ...
  const supportingBox = pending.source === 'replicate'
    ? state.objects.find((object) => object.kind === 'wooden-box' && object.position.x === to.x && object.position.y === to.y)
    : undefined;
  const destinationBlocked = state.objects.some((object) => object.position.x === to.x && object.position.y === to.y
    && object.kind !== 'wooden-box'
    && !(pending.source === 'replicate' && object.kind === 'spectre-replica' && object.ownerId === playerId));
  if (Object.values(state.players).some((player) => player.hp > 0 && player.position.x === to.x && player.position.y === to.y) || destinationBlocked) return fail(state, 'The replica requires an empty Square or a Wooden Box.');
  const replica = pending.source === 'split' ? createSpectreReplica(state, playerId, to) : createOrReplaceSpectreReplica(state, playerId, to, supportingBox?.id ?? null);
  ...
  if (pending.source === 'replicate' && pending.level >= 2) {
    const enemies = enemyBodies(state, playerId)
      .filter((enemy) => !state.players[enemy.ownerId].spectreShadowCloakActive && distance(replica.position, enemy.position) <= 2)
      .sort((a, b) => a.id.localeCompare(b.id));
    for (const enemy of enemies) {
      const from = { ...enemy.position };
      if (distance(replica.position, from) === 1) {
        state.log.unshift(`Replicate found ${enemy.name} already adjacent to the replica.`);
        continue;
      }
      const pushTarget = pushEntityForBody(enemy);
      const destination = shortestPullPath(state, pushTarget, replica.position)[0];
      if (!destination) {
        state.log.unshift(`Replicate could not pull ${enemy.name} toward the replica because no legal route was available.`);
        continue;
      }
      if (enemy.kind === 'player') {
        const moved = state.players[enemy.ownerId];
        recordQuestMovement(state, moved.id, 1, false, destination);
        moved.position = destination;
        moved.visualMovement = { from, path: [{ ...destination }], kind: 'replicate-pull', source: { ...replica.position }, tetherSource: { ...replica.position }, sourceObjectId: replica.id, sourcePlayerId: playerId, delayMs: 500, durationMs: 1000 };
        markCharacterMoved(moved, 'enemy-ability');
      } else moveBoardObject(state, state.objects.find((object) => object.id === enemy.id)!, destination);
      applyElevationDropDamage(state, pushTarget, from, destination, playerId, 'perk');
      state.log.unshift(`Replicate pulled ${enemy.name} 1 Square toward the replica, to ${cellLabel(destination)}.`);
    }
    const adjacentEnemies = enemyBodies(state, playerId).filter((enemy) => !state.players[enemy.ownerId].spectreShadowCloakActive && distance(replica.position, enemy.position) === 1);
    if (pending.level >= 3) {
      for (const enemy of adjacentEnemies) addForcedStatusCard(state, state.players[enemy.ownerId], 'panic', 'hand', playerId, 'perk', true);
      state.log.unshift(`Replicate added Panic to ${adjacentEnemies.length > 0 ? adjacentEnemies.map((enemy) => enemy.name).join(', ') : 'no enemies'} adjacent to the replica after the pull.`);
      for (const enemy of adjacentEnemies) damageCharacterBody(state, enemy, 1, false, playerId, 'perk');
      state.log.unshift(`Replicate level 3 dealt 1 Damage to ${adjacentEnemies.length > 0 ? adjacentEnemies.map((enemy) => enemy.name).join(', ') : 'no adjacent enemies'} after the pull.`);
    }
  }
...
function selectSpectrePerkOrigin(state: GameState, playerId: PlayerId, origin: 'spectre' | 'replica', replicaId?: string | null): CommandResult {
  ...
  if (origin === 'spectre' && pending.perkId !== 'shadow-dagger') return fail(state, `${cardDefinition({ instanceId: `${pending.perkId}-target`, cardId: pending.perkId }).name} must target one of Spectre’s replicas.`);
  const replicas = spectreReplicas(state, playerId);
  if (origin === 'replica' && state.players[playerId].traitBlocked) return fail(state, 'Curse prevents Spectre from targeting a replica with a Perk Card.');
  const selectedReplica = origin === 'replica' ? replicas.find((replica) => replica.id === replicaId) ?? (!replicaId ? replicas[0] : undefined) : undefined;
  if (origin === 'replica' && !selectedReplica) return fail(state, 'That replica is not available to use with this Perk.');
  pending.origin = origin;
  pending.replicaId = selectedReplica?.id ?? null;
  return ok(state);
}

const compassSteps: Cell[] = [
  { x: 0, y: -1 }, { x: 1, y: -1 }, { x: 1, y: 0 }, { x: 1, y: 1 },
  { x: 0, y: 1 }, { x: -1, y: 1 }, { x: -1, y: 0 }, { x: -1, y: -1 },
];

function closestEnemyFacingStep(state: GameState, character: PlayerState): Cell | null {
  ...
  for (const candidate of Object.values(state.players)) {
    if (candidate.id === character.id || candidate.hp <= 0) continue;
    const dx = candidate.position.x - character.position.x;
    const dy = candidate.position.y - character.position.y;
  ...
  const dx = closest.position.x - character.position.x;
  const dy = closest.position.y - character.position.y;
  return compassSteps.reduce((best, step) => {
    const score = (dx * step.x + dy * step.y) / Math.hypot(step.x, step.y);
    const bestScore = (dx * best.x + dy * best.y) / Math.hypot(best.x, best.y);
    return score > bestScore ? step : best;
  });
}

function hauntReplicaDestination(state: GameState, enemy: PlayerState): { position: Cell; boxId: string | null } | null {
  const facing = closestEnemyFacingStep(state, enemy);
  if (!facing) return null;
  const behind = { x: -facing.x, y: -facing.y };
  const behindIndex = compassSteps.findIndex((step) => step.x === behind.x && step.y === behind.y);
  const rankedSteps = compassSteps
    .map((step, index) => ({ step, clockwiseOffset: (index - behindIndex + compassSteps.length) % compassSteps.length }))
    .sort((a, b) => Math.min(a.clockwiseOffset, compassSteps.length - a.clockwiseOffset) - Math.min(b.clockwiseOffset, compassSteps.length - b.clockwiseOffset)
      || a.clockwiseOffset - b.clockwiseOffset)
    .map((entry) => entry.step);
  for (const step of rankedSteps) {
    const position = { x: enemy.position.x + step.x, y: enemy.position.y + step.y };
    if (position.x < 1 || position.x > boardWidth(state) || position.y < 0 || position.y >= boardHeight(state)) continue;
    if (Object.values(state.players).some((entry) => entry.hp > 0 && entry.position.x === position.x && entry.position.y === position.y)) continue;
    const objects = state.objects.filter((object) => object.position.x === position.x && object.position.y === position.y);
    if (objects.length === 0) return { position, boxId: null };
    if (objects.length === 1 && objects[0].kind === 'wooden-box') return { position, boxId: objects[0].id };
  }
  return null;
}

function resolveSpectreHaunt(state: GameState, player: PlayerState, level: number) {
  const enemies = Object.values(state.players).filter((enemy) => enemy.id !== player.id && enemy.hp > 0);
  state.objects = state.objects.filter((object) => object.kind !== 'spectre-replica' || object.ownerId !== player.id);
  const created: BoardObject[] = [];
  for (const enemy of enemies) {
    const destination = hauntReplicaDestination(state, enemy);
    if (destination) created.push(createSpectreReplica(state, player.id, destination.position, destination.boxId));
    ...
    if (level >= 2) {
      ...
      if (revealed) {
        ...
        state.log.unshift(`Haunt privately revealed and marked ${cardDefinition(revealed).name} in ${enemy.name}'s Hand; that Block Card must be used first.`);
      } else state.log.unshift(`Haunt found no Block Card in ${enemy.name}'s Hand.`);
    }
  }
  player.spectreAttackBonus = (player.spectreAttackBonus ?? 0) + 1;
  showStatEffect(state, player, 1, 'ATT');
  if (level >= 3) player.actionsRemaining += 1;
  state.log.unshift(`Haunt created ${created.length} replica${created.length === 1 ? '' : 's'}, granted +1 ATT until end of turn${level >= 3 ? ', and granted 1 Action' : ''}.`);
}
...
function resolveSpectreRelocate(state: GameState, player: PlayerState, replica: BoardObject, level: number): CommandResult {
  ...
  const lineVictims = enemiesBetweenCells(state, player.id, origin, replicaOrigin);
  for (const enemy of lineVictims) damageCharacterBody(state, enemy, 1, false, player.id, 'perk');
  state.log.unshift(`Relocate's tether dealt 1 Damage to ${lineVictims.length > 0 ? lineVictims.map((enemy) => enemy.name).join(', ') : 'no enemies'} on tiles intersected by the line.`);
  ...
  player.spectreRelocateTetherReplicaId = replica.id;
  ...
  if (level >= 2 && statuses.length > 0) {
    (state as SpectreTargetingState).spectreStatusChoice = { playerId: player.id, mode: 'relocate' };
    state.phase = 'choosing-blessed-prayer-discard';
    state.log.unshift(`Relocate swapped Spectre with the replica at ${cellLabel(player.position)}, granted +1 MOV, and requires 1 negative Status Card to be Removed.`);
  } else {
    state.phase = 'active';
    state.log.unshift(`Relocate swapped Spectre with the replica at ${cellLabel(player.position)}${level >= 2 ? ' and granted +1 MOV' : ''}${level >= 3 ? ' and 1 Action' : ''}.`);
  }
  return ok(state);
}
...
function confirmSpectrePerkOrigin(state: GameState, playerId: PlayerId): CommandResult {
  ...
  const caster = state.players[playerId];
  const replica = pending.origin === 'replica'
    ? spectreReplicas(state, playerId).find((candidate) => candidate.id === pending.replicaId)
    : undefined;
  ...
  extended.spectrePerkOrigin = null;
  if (pending.perkId === 'devour') {
    destroySpectreReplicaById(state, playerId, replica!.id);
    state.pendingAttack = null;
    state.phase = 'active';
    state.log.unshift(`Devour destroyed the selected replica at ${cellLabel(originPosition)}.`);
    return ok(state);
  }
  if (pending.perkId === 'relocate') return resolveSpectreRelocate(state, caster, replica!, pending.level);
  ...
  return fail(state, 'That Spectre Perk does not use origin confirmation.');
}
...
function resolveSpectreShadowDirection(state: GameState, playerId: PlayerId, to: Cell): CommandResult {
  ...
  const enemies = enemyBodies(state, playerId).filter((enemy) => !state.players[enemy.ownerId].spectreShadowCloakActive && trail.some((cell) => cell.x === enemy.position.x && cell.y === enemy.position.y));
  for (const enemy of enemies) {
    const owner = state.players[enemy.ownerId];
    if (pending.level >= 3) {
      owner.spectreShadowDefensePenalty = (owner.spectreShadowDefensePenalty ?? 0) + 1;
      showStatEffect(state, owner, -1, 'DEF');
    }
    if (pending.level >= 2) {
      const damageEventStart = state.objectPushAnimations.length;
      damageCharacterBody(state, enemy, 1, false, playerId, 'perk');
  ...
  if (trail.length > 0) state.spellProjectiles.push({ id: projectileId, casterId: playerId, targetId: `shadow-trail-${playerId}`, from: { ...origin }, to: { ...trail[trail.length - 1] }, path: trail.map((cell) => ({ ...cell })), count: 1, damage: pending.level >= 2 ? 1 : 0, style: 'missile' });
  state.phase = 'active';
  state.log.unshift(`Shadow Dagger from ${pending.origin === 'replica' ? 'the replica' : 'Spectre'} travelled to ${trail.length > 0 ? cellLabel(trail[trail.length - 1]) : cellLabel(origin)} and hit ${enemies.length} enem${enemies.length === 1 ? 'y' : 'ies'}${pending.level >= 3 ? ', reducing their next Defend Card by 1' : ''}.`);
  return ok(state);
}
...
function resolveSpectreStatusChoice(state: GameState, playerId: PlayerId, cardInstanceId: string | null): CommandResult {
  ...
  if (choice.mode === 'anguish' && choice.attackerId && state.pendingAttack && !state.pendingAttack.returnToHandAfterCombat) {
    const attacker = state.players[choice.attackerId];
    const restoredAttack = attacker.hand.find((card) => card.instanceId === state.pendingAttack!.cardInstanceId);
    if (restoredAttack) discardFromHand(attacker, restoredAttack.instanceId);
  }
...
function beginPendingSplit(state: GameState) {
  const extended = state as SpectreTargetingState;
  const pending = extended.pendingSplit;
  if (!pending || state.phase !== 'active' || state.combatReveal) return;
  extended.pendingSplit = null;
  const defender = state.players[pending.defenderId];
  if (!defender) { state.pendingAttack = null; return; }
  const hasLegalSquare = Array.from({ length: 9 }, (_, index) => ({ x: pending.origin.x + index % 3 - 1, y: pending.origin.y + Math.floor(index / 3) - 1 }))
    .some((cell) => distance(cell, pending.origin) === 1 && cell.x >= 1 && cell.x <= boardWidth(state) && cell.y >= 0 && cell.y < boardHeight(state)
      && hasLineOfSight(state, pending.origin, cell)
      && !Object.values(state.players).some((player) => player.hp > 0 && player.position.x === cell.x && player.position.y === cell.y)
      && !state.objects.some((object) => object.position.x === cell.x && object.position.y === cell.y));
  if (hasLegalSquare) {
    extended.spectreReplicaPlacement = { casterId: defender.id, level: 1, range: 1, source: 'split', origin: { ...pending.origin }, undo: null };
    state.phase = 'choosing-spirit-guardian-square';
    state.log.unshift(`Split: ${defender.name} must create a replica within Range 1 of Spectre.`);
  } else {
    state.pendingAttack = null;
    state.log.unshift('Split found no legal adjacent Square and created no replica.');
  }
}

function beginPendingAnguish(state: GameState) {
  const extended = state as SpectreTargetingState;
  const pending = extended.pendingAnguish;
  if (!pending || state.phase !== 'active' || state.combatReveal) return;
  extended.pendingAnguish = null;
  const defender = state.players[pending.defenderId];
  const attacker = state.players[pending.attackerId];
  if (!defender || !attacker) { state.pendingAttack = null; return; }
  if (pending.sufferedDamage) {
    const drawn = drawCards(defender, 1);
    state.log.unshift(`Anguish drew ${drawn} Card for ${defender.name} after suffering Damage.`);
  }
  if (attacker.hp > 0 && defender.hand.some(isNegativeStatusCard)) {
    extended.spectreStatusChoice = { playerId: defender.id, mode: 'anguish', attackerId: attacker.id };
    state.phase = 'choosing-blessed-prayer-discard';
    state.log.unshift(`Anguish: ${defender.name} may transfer 1 negative Status Card to ${attacker.name}.`);
  } else state.pendingAttack = null;
}
...
function moveDanceThrough(state: GameState, player: PlayerState, to: Cell): CommandResult {
  ...
  const passedEnemy = passedReplica ?? (dance.enemyUnderfoot ? characterBody(state, 'player', dance.enemyUnderfoot) : null);
  const enteredFrom = { ...player.position };
  recordQuestMovement(state, player.id, 1, false, to);
  ...
  danceWithTransit.objectUnderfoot = targetObject?.id ?? null;
  state.log.unshift(`${player.name} danced to ${cellLabel(to)} (${dance.stepsRemaining} steps left).`);
  if (passedEnemy) {
    const danceWithPins = danceWithTransit;
    danceWithPins.pinnedBodyIds ??= [];
...
function endDance(state: GameState, playerId: PlayerId): CommandResult {
  ...
  if ((state.danceThrough as typeof state.danceThrough & { objectUnderfoot?: string | null }).objectUnderfoot) return fail(state, 'Obi Wan Shinobi must leave the Object-occupied Square.');
...
function moveDoubleJump(state: GameState, player: PlayerState, to: Cell): CommandResult {
  ...
  const passedEnemy = passedReplica ?? (jump.enemyUnderfoot ? characterBody(state, 'player', jump.enemyUnderfoot) : null);
  const enteredFrom = { ...player.position };
  recordQuestMovement(state, player.id, 1, false, to);
  ...
  jumpWithTransit.objectUnderfoot = targetObject?.id ?? null;
  state.log.unshift(`${player.name} double-jumped to ${cellLabel(to)} (${jump.stepsRemaining} steps left).`);
  if (passedEnemy) {
    const jumpWithPins = jumpWithTransit;
    jumpWithPins.pinnedBodyIds ??= [];
    const bodyKey = `${passedEnemy.kind}:${passedEnemy.id}`;
...
function resolveFinishingDiscard(state: GameState, playerId: PlayerId, cardInstanceId: string): CommandResult {
  if (playerId !== state.activePlayerId || !['choosing-guard-discard', 'choosing-dash-discard', 'choosing-end-discard', 'choosing-hot-potato-discard'].includes(state.phase as string)) return fail(state, 'No discard is pending.');
  const player = state.players[playerId];
  const card = player.hand.find((entry) => entry.instanceId === cardInstanceId);
  if (!card) return fail(state, 'That card is not in the hand.');
  if (cardDefinition(card).cannotBeDiscarded) return fail(state, `${cardDefinition(card).name} cannot be discarded.`);
  if ((state.phase === 'choosing-guard-discard' || state.phase === 'choosing-dash-discard') && card.cardId === 'judgement') return fail(state, 'Judgement cannot be discarded to pay for Guard or Dash.');
  ...
  if ((state.phase as string) === 'choosing-hot-potato-discard') {
    const potato = questPhases(state).hotPotato;
    if (potato) potato.endDiscardCompletedTurn = `${playerId}:${state.turn}`;
    state.phase = 'active';
    return ok(endTurn(state));
  }
...
function endTurn(state: GameState): GameState {
  ...
  if (hotPotato?.carrierId === current.id && hotPotato.endDiscardCompletedTurn !== `${current.id}:${state.turn}`) {
    const discardable = current.hand.filter((card) => card.cardId !== 'hot-potato' && !cardDefinition(card).cannotBeDiscarded);
    if (discardable.length > 0) {
      state.phase = 'choosing-hot-potato-discard' as GamePhase;
      state.log.unshift(`${current.name} holds the Hot Potato and must discard 1 Card before ending the turn.`);
      return state;
    }
    removeHotPotatoCard(current);
    hotPotato.carrierId = null;
    hotPotato.endDiscardCompletedTurn = `${current.id}:${state.turn}`;
    state.log.unshift(`${current.name} had no Card to discard, lost the Hot Potato, and returned it to the center of the arena.`);
  }
  ...
  const expiredJudgements = current.hand.filter((card) => card.cardId === 'judgement');
  for (const judgement of expiredJudgements) removeCard(current, judgement.instanceId);
  if (expiredJudgements.length > 0) state.log.unshift(`Judgement was automatically Removed at the end of ${current.name}'s turn.`);
  ...
  if (current.character === 'spectre') {
    ...
    current.spectreRelocateTetherReplicaId = null;
    current.spectreAccumulateActive = 0;
    current.spectreShadowMoveBonus = 0;
    (state as SpectreTargetingState).spectreShadow = null;
    for (const player of Object.values(state.players)) {
      player.spectreShadowDefensePenalty = 0;
    }
    state.log.unshift(`${current.name}'s Shadow trail and temporary ATT effects expired at the end of the turn.`);
  }
  ...
  if (current.traitBlocked) {
    current.traitBlocked = false;
    state.log.unshift(`Curse expired and restored ${current.name}'s Trait at the end of their turn.`);
  }
...
function finalizeTurn(state: GameState): GameState {
  ...
  if (endingQuest.currentQuest?.id === 'the-spy') {
    const currentPlayer = state.players[state.activePlayerId];
    const position = cellLabel(currentPlayer.position);
    const endedOnEnemyBase = (Object.keys(state.players) as PlayerId[]).some((playerId) => playerId !== currentPlayer.id && ownedBaseSquares(state, playerId).has(position));
    if (endedOnEnemyBase) {
      endingQuest.currentQuest.progress[currentPlayer.id] = (endingQuest.currentQuest.progress[currentPlayer.id] ?? 0) + 1;
      state.log.unshift(`${currentPlayer.name} ended the turn on an enemy Base Square, gaining 1 The Spy progress.`);
    }
  }
  ...
  const nextRespawn = endingQuest.objectRespawns?.[0];
  if (beginsNewRound && nextRespawn?.dueRound !== null && nextRespawn?.dueRound !== undefined && nextRespawn.dueRound <= state.turn) {
    if (spawnReplacementBox(state)) {
      endingQuest.objectRespawns!.shift();
      const nextDelay = activateNextObjectRespawn(state);
      if (nextDelay !== null) state.log.unshift(`The next destroyed starting Object will respawn in ${nextDelay} Rounds.`);
    } else {
      nextRespawn.dueRound = state.turn + 1;
      state.log.unshift('The next Object respawn was delayed by 1 Round because no eligible Square was empty.');
    }
  }
  ...
  if (next.character === 'spectre') {
    ...
    if (next.spectreAccumulateActive > 0) {
      showStatEffect(state, next, next.spectreAccumulateActive, 'ATT');
      state.log.unshift(`Accumulate granted ${next.name} +${next.spectreAccumulateActive} ATT for this turn.`);
    }
  }
  if (next.character === 'john-christ') {
    ...
    next.stoicShellStacks = 0; // Retained for saved-state compatibility; healing no longer stacks.
    if (beganWithStoicShell && next.hp < next.maxHp && !next.traitBlocked) {
      ...
      state.log.unshift(`${next.name}'s Stoic Shell restored ${healed} HP. Stoic Shell remains until HP Damage removes it.`);
    } else if (beganWithStoicShell && !next.traitBlocked) {
      state.log.unshift(`${next.name} began the turn at maximum HP, so Stoic Shell remained active without healing.`);
    }
    if (next.queuedBlessingCardIds.length > 0) {
      const queued = next.queuedBlessingCardIds.splice(0);
      for (const cardId of queued) addBlessingCardToJohn(state, next, cardId, 'block');
    }
  }
...
function resolveManaChoice(state: GameState, playerId: PlayerId, consume: boolean): CommandResult {
  ...
  if (consume) {
    if (player.traitBlocked) return fail(state, 'Curse blocks Classic Wizardry, so stored Mana cannot trigger Consume.');
    ...
    state.log.unshift(`${player.name} consumed 3 Mana: +1 Attack Range this turn. Attack and Perk spells gain their advanced effects, but normal spell resolution cannot generate Mana.`);
  } else {
...
export function grantMana(player: PlayerState, amount = 1): number {
  if (player.character === 'magician' && player.traitBlocked) return 0;
...
export function drawCards(player: PlayerState, count: number): number {
  ...
  for (let index = 0; index < count; index++) {
    if (player.deck.length === 0 && player.discard.length > 0) { player.deck = shuffle(player.discard.splice(0)); player.knownTopCardId = null; player.knownTopCardIds = []; }
    const card = player.deck.pop();
    if (!card) break;
    if (player.knownTopCardIds?.[0] === card.cardId) player.knownTopCardIds.shift();
    else player.knownTopCardIds = [];
    player.knownTopCardId = player.knownTopCardIds[0] ?? null;
    player.hand.push(card);
...
function teleportTestObject(state: GameState, playerId: PlayerId, objectId: string, to: Cell): CommandResult {
  ...
  moveBoardObject(state, object, to);
...
export function markCharacterMoved(player: PlayerState, cause: 'voluntary' | 'own-card' | 'enemy-ability') {
  ...
  if (cause === 'voluntary' && !player.lightsaberMovementProtection) {
...
export function effectiveMoveRange(player: PlayerState): number {
  if (player.character === 'john-christ' && player.spiritForm && !player.traitBlocked) return 1;
  return normalEffectiveMoveRange(player);
}
function normalEffectiveMoveRange(player: PlayerState): number {
  const boomerangMovePenalty = [...(player.deck ?? []), ...(player.discard ?? [])].some((card) => card.cardId === 'boomerang' || card.cardId === 'boomerang-draw') ? 1 : 0;
...
export function applyPinned(player: PlayerState, stacks = 1, state?: GameState, sourceId?: PlayerId): number {
  const previousMoveRange = movementRangeForAdjustment(player);
  const requested = Math.max(0, Math.floor(stacks));
  let applied = 0;
  for (let index = 0; index < requested; index++) {
    if (state) applied += Number(addForcedStatusCard(state, player, 'pinned', 'hand', sourceId ?? state.activePlayerId, 'other', true));
    else if (forcedStatusCount(player, 'pinned') < 5) {
      player.hand.push({ instanceId: `${player.id}-status-${++instanceSequence}`, cardId: 'pinned', revealedToOpponent: true });
      applied += 1;
    }
  }
...
function updateLightsaberAtTurnEnd(state: GameState, player: PlayerState) {
  ...
  if (player.traitBlocked) {
    if (player.swiftformLightsaberAtTurnEnd) {
      player.lightsaberBuff = true;
      player.lightsaberAppliedWhileTraitBlocked = true;
      state.log.unshift('Swiftform applied Lightsaber status while Curse suppressed its bonuses.');
    }
    if (!player.lightsaberAppliedWhileTraitBlocked) {
      player.lightsaberBuff = false;
      player.lightsaberStacks = 0;
      state.log.unshift('Curse prevented the Lightsaber trait from empowering or prolonging Obi Wan Shinobi.');
    } else if (!player.movedThisTurn) state.log.unshift('A Lightsaber effect applied while Cursed remained active because Obi Wan Shinobi did not move.');
    else {
      player.lightsaberBuff = false;
      player.lightsaberStacks = 0;
      state.log.unshift('The Lightsaber effect applied while Cursed expired because Obi Wan Shinobi moved.');
    }
    player.lightsaberAppliedWhileTraitBlocked = false;
    player.lightsaberMovementProtection = false;
    return;
  }
  if (player.movedThisTurn) {
    ...
    if (!player.lightsaberBuff) {
      state.log.unshift('Lightsaber empowered Obi Wan Shinobi with +1 ATT, +1 DEF, and +1 MOV.');
      showLightsaberStats(state, player);
    }
    player.lightsaberBuff = true;
  }
  if (player.swiftformLightsaberAtTurnEnd) {
...
function removeNegativeHandEffects(player: PlayerState): number {
  const previousMoveRange = movementRangeForAdjustment(player);
  const negativeStatusIds: CardTypeId[] = ['pinned', 'headache', 'exhaust', 'burning', 'panic'];
  const removed = player.hand.filter((card) => negativeStatusIds.includes(card.cardId)).length;
  player.hand = player.hand.filter((card) => !negativeStatusIds.includes(card.cardId));
  player.pinnedStacks = player.hand.filter((card) => card.cardId === 'pinned').length;
  player.pinnedGainedThisTurn = Math.min(player.pinnedGainedThisTurn, player.pinnedStacks);
  adjustUnspentMovementForRangeChange(player, previousMoveRange);
  return removed;
}
...
function discardFromHand(player: PlayerState, instanceId: string) {
  ...
  if (card.cardId === 'boomerang' || card.cardId === 'boomerang-draw') {
