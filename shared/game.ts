...
function nextRedirectObject(state: GameState, defender: PlayerState): BoardObject | undefined {
  ...
  return state.objects.find((object) => isRedirectDestructibleObject(object) && !redirect.usedObjectIds.includes(object.id) && distance(object.position, defender.position) === 1);
}
// Redirect may destroy character-created Wall Objects. Only permanent arena
// walls and invincible Spirit Guardians are excluded; Shields and Tombs are
// deliberately eligible even though they block movement and line of sight.
function isRedirectDestructibleObject(object: BoardObject): boolean {
  return object.kind !== 'wall-pillar' && !isGuardianWall(object);
}
...
function applyPerkEffects(state: GameState, player: PlayerState, perk: Card, level: number) {
  ...
  if (perk.id === 'windwalker-stance') {
    ...
    grantMovement(player, Math.max(0, player.windwalkerMoveBonus - previousBonus));
  ...
  if (perk.id === 'carian-stance') {
    ...
    if (level >= 2) player.merylinSummonedDefenseBonus = 1;
  ...
  if (perk.id === 'consume-replica') {
    ...
    if (level >= 3 && removedEffects > 0) player.spectreAttackBonus = (player.spectreAttackBonus ?? 0) + removedEffects;
  ...
  if (!pending.defensePreCombatResolved) {
    ...
    if (defenseCardId === 'redirect' && !defenseEffectsCancelled && defender.character === 'merylin') {
      const preparedObjectIds = state.objects
        .filter((object) => isRedirectDestructibleObject(object) && distance(object.position, defenderCombatPosition) === 1)
        .slice(0, 3)
...
function resolveSpectreRelocate(state: GameState, player: PlayerState, replica: BoardObject, level: number): CommandResult {
  ...
  markCharacterMoved(player, 'own-card');
  if (level >= 2) grantMovement(player, 1);
  if (level >= 3) player.actionsRemaining += 1;
