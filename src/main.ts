...
let forfeitedCombatTargetId: PlayerId | null = null;
...
function combatAnimationImpactPending() {
  return Boolean(pendingFlurryAttack) || merylinCombatImpactPending || obiWanCombatImpactPending || orkkCombatImpactPending || johnCombatImpactPending || loganCombatImpactPending || performance.now() < genericCombatImpactAt;
}
...
function resetCombatSummary() {
  ...
  completedAttackEffectAnimations.clear();
...
function characterSelectButton(character: HotseatCharacter, dataAttribute: 'data-hotseat-character' | 'data-character', disabled = false, selectionFrames = ''): string {
  ...
  const portraitMarkup = portrait ? `<img src="${portrait}" alt="" draggable="false">` : '';
  const trait = info.trait ? `<small class="character-trait-stat"><span class="character-select-trait-icon" tabindex="0" aria-label="${info.trait}: ${info.traitDescription}">${gameIcon(info.traitIcon)}<span class="character-select-trait-tooltip"><b>${info.trait}</b>${info.traitDescription}</span></span><span>${info.trait}</span></small>` : '<small class="character-trait-stat">TRAIT COMING LATER</small>';
  return `<button class="character-choice-card" data-profile-character="${character}" ${dataAttribute}="${character}" ${disabled ? 'disabled' : ''}>${selectionFrames}<strong>${info.name}</strong><span class="character-choice-layout"><span class="character-choice-portrait" aria-hidden="true">${portraitMarkup}</span><span class="character-core-stats"><small><b>${info.hp}</b> MAX HP</small><small><b>${info.movement}</b> MOV</small><small><b>${info.attackRange}</b> ${character === 'merylin' ? 'MELEE' : 'ATT RANGE'}</small>${trait}</span></span></button>`;
}

function dummySelectButton(): string {
  return `<button class="character-choice-card" data-hotseat-opponent="dummy"><strong>Test Dummy</strong><span class="character-choice-layout"><span class="character-choice-portrait" aria-hidden="true"></span><span class="character-core-stats"><small><b>20</b> MAX HP</small><small><b>2</b> MOV</small><small><b>2</b> ATT RANGE</small><small class="character-trait-stat"><span>TRAINING OPPONENT</span></small></span></span></button>`;
}
...
function actingPlayer(): PlayerId {
  ...
  if (gameState.phase === 'choosing-sacrifice-tomb-square') return (gameState as GameState & { sacrificeTomb?: { casterId: PlayerId } }).sacrificeTomb?.casterId ?? gameState.activePlayerId;
...
function renderUI() {
  ...
  if (gameState.phase === 'choosing-blink-discard') prompt.textContent = 'Blink: discard another Card from Hand as an extra cost';
  ...
  const canPayForDash = actor.hand.some((card) => card.cardId === 'burning' || (!cardDefinition(card).cannotBeDiscarded && !cardDefinition(card).name.startsWith('Blessing:') && card.cardId !== 'judgement'));
...
function renderMatchResults() {
  ...
  if (matchEndPresentation?.phase !== 'ready') {
    modal.classList.add('hidden');
    modal.innerHTML = '';
    return;
  }
...
async function downloadCombatSummary() {
  ...
  button.textContent = 'Preparing CSV summary…';
  ...
  try {
    const csv = buildCombatSummaryCsv(summary);
    ...
    button.textContent = 'CSV summary downloaded';
  } catch (error) {
    console.error('Could not export the combat summary.', error);
    button.textContent = 'CSV export failed — retry';
  } finally {
    button.disabled = false;
    setTimeout(() => { if (button.isConnected) button.textContent = 'Download CSV summary'; }, 2_500);
  }
}
...
function playerUiColor(playerId: PlayerId) {
  return playerId === 'P1' ? '#5e54ff' : playerId === 'P2' ? '#d11f4f' : '#50c878';
}
...
function playerStatusIcons(player: GameState['players'][PlayerId]) {
    ...
    const handExhaustIcon = exhaustInHand > 0 ? `<div class="status-icon exhaust-status in-hand" tabindex="0">${gameIcon('exhaust')}${exhaustInHand > 1 ? `<b>${exhaustInHand}</b>` : ''}<span class="status-tooltip"><strong>Exhaust · Hand</strong>Cards have -1 Attack and Defend Value while Exhaust is held. During combat, one may be Removed for a -2 modifier instead.</span></div>` : '';
...
function renderHand() {
  ...
  byId('hand').innerHTML = viewer.hand.map((instance) => {
    ...
    const cannotOverstackDiscard = !mindTricksReveal && choosingDiscard && (card.cannotBeDiscarded || ((gameState.phase === 'choosing-guard-discard' || gameState.phase === 'choosing-dash-discard') && instance.cardId === 'judgement') || (gameState.phase === 'choosing-dash-discard' && card.name.startsWith('Blessing:')) || (gameState.phase === 'choosing-blink-discard' && (instance.cardId === 'pinned' || instance.instanceId === (gameState.pendingAttack as typeof gameState.pendingAttack & { blinkDefenseCommand?: { cardInstanceId: string } })?.blinkDefenseCommand?.cardInstanceId)) || (gameState.phase === 'choosing-end-discard' && card.kind === 'status' && card.canDiscardForHandLimit !== true));
    ...
    const removeOnUseOrDiscard = instance.cardId === 'feint' || instance.cardId === 'weak-feint' || instance.cardId === 'judgement';
...
function renderFlurryModal() {
  ...
  if (gameState.phase === 'choosing-frostmourne' && frostmourne) {
    ...
    modal.classList.toggle('hidden', !visible);
    if (!visible) { modal.innerHTML = ''; return; }
    modal.innerHTML = `<div class="choice-dialog"><span>FROSTMOURNE · AFTER COMBAT</span><h2>Feed the Blade?</h2><p>Sacrifice 1 Hit Point to put Frostmourne on top of your Deck, gain 1 Action, and Summon.</p><div class="choice-cards"><button id="frostmourneUse"><strong>Sacrifice 1 HP</strong><small>Frostmourne to top of Deck · Gain 1 Action · Summon</small></button></div><button class="choice-decline" id="frostmourneDecline">Leave Frostmourne in Discard</button></div>`;
  ...
  if (gameState.phase === 'mana-blast-offer' && gameState.pendingAttack?.feedSpiritOffered) {
    ...
    const combatHealing = gameState.pendingAttack.feedSpiritCombatDamage ?? 0;
    modal.innerHTML = `<div class="choice-dialog"><span>DEFENCE FOLLOW-UP</span><h2>Feed the Spirit</h2><p>You may Remove one Blessing Card to restore ${combatHealing} Hit Point${combatHealing === 1 ? '' : 's'} lost from combat Damage in this combat.</p><div class="choice-cards">${blessings.map((instance) => `<button data-feed-blessing="${instance.instanceId}"><strong>${escapeHtml(cardDefinition(instance).name)}</strong><small>Remove · Restore up to ${combatHealing} HP</small></button>`).join('')}</div><button class="choice-decline" id="feedSpiritDecline">Do not remove · Esc</button></div>`;
  ...
  modal.innerHTML = `<div class="choice-dialog"><span>BEFORE COMBAT</span><h2>Flurry</h2><p>Lose 1 HP to force ${escapeHtml(gameState.players[flurry.attackerId].name)} to discard 1 Card before combat.</p><div class="choice-cards"><button id="flurryPayHp"><strong>Lose 1 HP</strong><small>Force the Attacker to discard 1 Card</small></button></div><button class="choice-decline" id="flurryDecline">Do not activate</button></div>`;
...
function renderFocusModal() {
  ...
  const simultaneousFfaSetup = gameState.boardSize === LORDAERON_ARENA.height;
  const playerId = simultaneousFfaSetup && mode === 'online' && localSeat && opening?.pendingPlayerIds.includes(localSeat)
    ? localSeat
    : opening?.pendingPlayerIds[0];
  const visible = Boolean(playerId) && (gameState.phase === 'choosing-focus' || gameState.phase === 'choosing-focus-card') && canLocalAct(playerId!);
  modal.classList.toggle('hidden', !visible);
  if (!visible || !playerId || !opening) { modal.innerHTML = ''; return; }
  const player = gameState.players[playerId];
  if (!opening.focusByPlayer[playerId]) {
...
function renderCombatReveal() {
  ...
  if (!reveal) {
    ...
    if (combatRevealWasVisible) {
      ...
      if (swing || obiWanAttack || orkkAttack || johnAttack || loganCombatAttacker) {
        ...
        if (loganCombatAttacker) loganCombatImpactPending = true;
    ...
    if (pendingFlurryAttack) return;
    const blink = blinkCombatPresentation;
    const targetFor = (defenderId: PlayerId) => blink ? worldPosition(blink.from) : dummyGroups.get(defenderId)?.position ?? worldPosition(gameState.players[defenderId].position);
    const startAttackPresentation = () => {
    const loganAttack = loganCombatAttacker;
    if (loganAttack) {
      loganCombatAttacker = null;
  ...
  modal.classList.toggle('has-attack-portrait', Boolean(portraitAttack));
  modal.classList.toggle('has-defend-portrait', Boolean(portraitDefend));
  lastCombatSummaryOpen = false;
  ...
  const attackRulesHtml = cardRulesHtml(attack, false);
  ...
  if (gameState.phase === 'choosing-combat-stack' && gameState.pendingAttack && (localSeat || mode === 'hotseat')) {
    ...
    if (mode === 'online' && localSeat && !combatants.includes(localSeat)) {
    ...
    const optionResult = (instance: (typeof player.hand)[number]) => {
      ...
      if (instance.cardId === 'exhaust') return `${attacker ? 'ATT' : 'DEF'} ${ownValue} → ${ownValue - 1} · attached Exhaust replaces its held -1 with -2`;
    ...
    const cardButtons = applicable.map((instance) => {
      const card = cardDefinition(instance);
      const shortEffect: Partial<Record<CardTypeId, string>> = { exhaust: 'Attach for -2 to your played Card.', 'vicious-mockery': '+2 to your played Card.', 'vicious-mockery-1': '+1 to your played Card.', banner: '+1 to your played Card.', 'mythril-helmet': 'Negate all Damage.', 'blessing-light': '-1 to enemy Defend.', 'blessing-might': '+2 to your Attack.', 'blessing-shield': 'Block 1 combat or effect Damage and 1 Status.', 'blessing-faith': 'Negate all Damage to both sides.' };
  ...
  if (reveal.exhaust) {
    ...
    modal.innerHTML = `<div class="combat-reveal-dialog"><span>COMBAT MODIFIER</span><h2>${escapeHtml(gameState.players[decisionPlayer].name)}: attach Exhaust?</h2><div class="combat-reveal-cards"><article class="combat-card attack"><label>ATTACK VALUE <strong>${modifier(reveal.attackBase, reveal.attackTotal)}</strong></label><div><span>ATTACK</span><h3>${escapeHtml(attack.name)}</h3><b>${reveal.attackTotal}</b><small>${attackRulesHtml}</small></div></article>${defendCard}</div><div class="combat-ack-status">Remove one Exhaust from Hand and apply -2 Value to your played card, or keep its normal -1 penalty.</div><div class="combat-choice-buttons"><button id="attachExhaust" ${mayDecide ? '' : 'disabled'}>ATTACH · -2</button><button id="keepExhaust" ${mayDecide ? '' : 'disabled'}>KEEP · -1</button></div></div>`;
  ...
  const readyLabel = viewer && viewerParticipates ? `${escapeHtml(gameState.players[viewer].name)}: READY` : viewer ? 'SPECTATING COMBAT' : 'OK';
...
function cardRulesHtml(card: ReturnType<typeof cardDefinition>, includeFallback = true): string {
  const translation = hintsLanguage === 'ru' ? CARD_RULES_RU[card.id] : undefined;
  const levelEffects = translation?.levelEffects ?? card.levelEffects;
  ...
  return includeFallback ? escapeHtml(cardRulesText(card)) : '';
}
...
const completedAttackEffectAnimations = new Set<string>();
...
const repentFireAnimations: { animationId: string; fire: JohnRepentFire }[] = [];
const holyFireAnimations: { animationId: string; group: THREE.Group; flames: THREE.Mesh[]; ring?: THREE.Mesh; startedAt: number; duration: number; style: 'holy-fire' | 'cleanse-immolate' }[] = [];
...
type MatchEndPresentation = {
  key: string;
  defeatedId: PlayerId | null;
  phase: 'effects' | 'death' | 'delay' | 'ready';
  deathEndsAt?: number;
  revealAt?: number;
};
...
renderer.setAnimationLoop((time) => {
  ...
  if (genericCombatImpactAt && time >= genericCombatImpactAt) {
    genericCombatImpactAt = 0;
    if (!merylinCombatImpactPending && !obiWanCombatImpactPending && !orkkCombatImpactPending && !johnCombatImpactPending && !loganCombatImpactPending) showMissedCombatCallout();
    syncBoard();
  ...
  if (manaShieldVisual) {
    const { effect, defenderId, replicaId } = manaShieldVisual;
    const target = replicaId ? objectGroups.get(replicaId) : dummyGroups.get(defenderId);
...
function updateCharacterHealthBars(refreshContents = false) {
  ...
  playerIds.forEach((playerId) => {
    ...
    const fill = playerUiColor(playerId);
...
function spawnJohnFireImpact(event: { id: string; style?: string; to: Cell; targetId?: string }) {
  ...
  holyFireAnimations.push({ animationId: event.id, group, flames, ring, startedAt: performance.now(), duration: cleanseImmolate ? 1450 : 2000, style: cleanseImmolate ? 'cleanse-immolate' : 'holy-fire' });
}
  ...
  for (const event of gameState.spellProjectiles ?? []) {
    ...
    if (event.style === 'holy-fire' || event.style === 'cleanse-immolate' || event.style === 'repent-fire') {
      if (event.style === 'repent-fire') {
        ...
        repentFireAnimations.push({ animationId: event.id, fire });
...
function updateSpellProjectiles(time: number) {
  let completedAttackEffectThisFrame = false;
  for (let index = fireboltAnimations.length - 1; index >= 0; index--) {
    ...
    if (complete) {
      visual.dispose();
      completedAttackEffectAnimations.add(animationId);
      completedAttackEffectThisFrame = true;
  ...
  for (let index = spellProjectileAnimations.length - 1; index >= 0; index--) {
    ...
    if (progress >= 1) {
      ...
      completedAttackEffectAnimations.add(animation.animationId);
      completedAttackEffectThisFrame = true;
    }
  }
  ...
  for (let index = repentFireAnimations.length - 1; index >= 0; index--) {
    const { animationId, fire } = repentFireAnimations[index];
    if (fire.update(time)) {
      fire.dispose();
  ...
  if (completedAttackEffectThisFrame && gameState.objectPushAnimations.some((event) => event.arcaneBarrier)) syncBoard();
}
...
function releaseBlessingPresentation(presentation: BlessingPresentation) {
  ...
  renderUI();
  if (gameState.objectPushAnimations.some((event) => event.arcaneBarrier?.waitForAttackEffectIds?.includes(presentation.id))) syncBoard();
}
...
function ensureMatchEndPresentation() {
  if (gameState.phase !== 'finished') return;
...
function finishingBlowVisualsActive(time: number) {
  ...
  for (const group of dummyGroups.values()) {
    ...
    if (spectre?.oneShot) return true;
  }
  return false;
}
...
function updateMatchEndPresentation(time: number) {
  ...
  if (presentation.phase === 'effects') {
    ...
    if (deathEndsAt) {
      ...
      presentation.deathEndsAt = deathEndsAt;
    } else {
      presentation.phase = 'delay';
      presentation.revealAt = time + 1000;
    }
    return;
  }
  if (presentation.phase === 'death') {
    if (time < (presentation.deathEndsAt ?? time)) return;
    presentation.phase = 'delay';
    presentation.revealAt = time + 1000;
    return;
  }
  if (presentation.phase === 'delay' && time >= (presentation.revealAt ?? time)) {
...
function arcaneBarrierWaiting(event: GameState['objectPushAnimations'][number]): boolean {
  return Boolean(event.arcaneBarrier && (
    ...
    || combatAnimationImpactPending()
    || event.arcaneBarrier.waitForAttackEffectIds?.some((id) => !completedAttackEffectAnimations.has(id))
  ));
}

function syncBoard() {
  syncBlessingPresentationEvents();
  const waitingBarrier = gameState.objectPushAnimations.find((event) => event.arcaneBarrier && arcaneBarrierWaiting(event));
  const arenaId = visualArena().id;
  ...
  (Object.keys(gameState.players) as PlayerId[]).forEach((id) => {
    ...
    const deferMovementUntilBarrier = previousKey !== undefined
      && previousKey !== targetKey
    ...
    if (!deferMovementUntilCombatImpact && !deferMovementUntilBarrier) lastVisualCells.set(id, targetKey);
  ...
  gameState.objects.forEach((object) => {
    ...
    const deferObjectUntilBarrier = previousKey !== undefined
      && previousKey !== targetKey
      && waitingBarrier?.arcaneBarrier?.targetObjectId === object.id
      && !waitingBarrier.collided;
    if (!previousKey) {
      if (object.kind === 'orkk-shield') settleOrkkShieldAtRest(group, object.ownerId, target);
      else group.position.copy(target);
    }
    else if (deferObjectUntilBarrier) {
      // Keep an attacking replica in the AoE until the attack visual finishes.
    }
    else if (previousKey !== targetKey) {
    ...
    if (!deferObjectUntilBarrier) lastObjectVisualCells.set(object.id, targetKey);
  ...
  gameState.objectPushAnimations.forEach((event) => {
    ...
    if (event.arcaneBarrier) {
      // Attack effects resolve first. Release the ward alongside the push.
      if (arcaneBarrierWaiting(event)) return;
    ...
    if (waitingBarrier?.arcaneBarrier?.targetObjectId === event.objectId && event.id.includes('-arcane-barrier-')) return;
    if (event.callout) {
      processedObjectPushAnimations.add(event.id);
    ...
    if (event.damage) {
      if (event.afterBarrierAnimationId && !processedObjectPushAnimations.has(event.afterBarrierAnimationId)) return;
...
function spectreAttackOriginForTarget(attacker: GameState['players'][PlayerId], target: Cell): 'spectre' | 'replica' | null {
  ...
  const candidates: Array<{ origin: 'spectre' | 'replica'; position: Cell; range: number }> = [
    ...
    ...(!attacker.traitBlocked ? spectreReplicas(gameState, attacker.id).map((replica) => ({ origin: 'replica' as const, position: replica.position, range: 1 })) : []),
  ];
...
function onBoardClick(event: MouseEvent) {
  ...
  } else if (gameState.phase === 'choosing-sacrifice-tomb-square') {
    const cellHit = hits.find((hit) => hit.object.userData.cell);
    const sacrificeTomb = (gameState as GameState & { sacrificeTomb?: { casterId: PlayerId } | null }).sacrificeTomb;
...
function renderCharacterBrowserCards() {
  ...
  byId('perkBrowserTrack').innerHTML = cards.map((card, index) => {
    ...
    const consumeText = 'consumeText' in card && card.consumeText ? card.consumeText : '';
    ...
    const rules = card.kind === 'perk' ? `<div class="character-perk-levels">${levelCopy}</div>${effectText ? `<p class="character-perk-extra">${escapeHtml(effectText)}</p>` : ''}` : `<p class="character-perk-extra character-card-description${consumeText ? ' has-consume' : ''}">${escapeHtml(effectText || 'No additional effect.')}</p>${consumeText ? `<p class="character-perk-extra character-consume-description">${escapeHtml(consumeText)}</p>` : ''}`;
